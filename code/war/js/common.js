...
function printCourseList(courseList, user)
{
	...
	{ 
		if(courseList[loop].status == "false" || courseViewArchivedStatus == courseViewArchived.show)
		{
			//common view:
			output = output + "<tr>";
			output = output + "<td id=\"courseID" + counter + "\">" + courseList[loop].ID + "</td>"; 
			output = output + "<td id=\"courseName" + counter + "\">" + sanitize(courseList[loop].name) + "</td>"; 
			//student view:
			if(user == STUDENT){
...
/*
 * helper: print course student list
 * TODO: Merge into printCourseCoordinatorForm
 * */
...
/*
 * Coordinator view student detail (individual record)
 * TODO: Improve UI, abstract print_detail function:: printCourseStudentForm
 * */
function printStudent(courseID, email, name, teamName, googleID, registrationKey, comments) {
	...
	var output = "<table class=\"detailform\">" +
				 "<tr>" +
...
/*
 * Coordinator edit student detail
 * TODO: Merge into printStudent
 * */
...
function printEvaluationAddForm(){
	...
	"<table class=\"headerform\">" +
		"<tr>" +
			...
			"<td><select style=\"width: 260px;\" name=\"" + COURSE_ID + "\" id=\"" + COURSE_ID + "\"" +
			...
			"<td><input style=\"width: 100px;\" type=\"text\" name=\"" + EVALUATION_START + "\" id=\"" + EVALUATION_START + "\" + " +
		...
		"<tr>" +
			...
			"<td><input style=\"width: 260px;\" type=\"text\" name=\"" + EVALUATION_NAME + "\" id=\"" + EVALUATION_NAME + 
			...
			"<td> <input style=\"width: 100px;\" type=\"text\" name=\"" + EVALUATION_DEADLINE + "\" id=\"" + EVALUATION_DEADLINE + "\" + " +
...
/*
 * helper: print edit evaluation form
 * TODO: Merge into printEvaluationAddForm
 * */
...
/*
 * helper: print evaluation list
 * TODO: abstract print dataform
 * */
function printEvaluationList(evaluationList) {
	...
	for (loop = 0; loop < evaluationList.length; loop++) { 
		output = output +
						...
						"<td class=\"t_eval_response centeralign\">" + evaluationList[loop].numberOfCompletedEvaluations + " / " + evaluationList[loop].numberOfEvaluations + "</td>"; 
		//display actions:
		output = output +
						"<td class=\"centeralign\">"
	...
	for (loop = 0; loop < evaluationList.length; loop++) {
		if (document.getElementById('editEvaluation' + loop) != null && document.getElementById('editEvaluation' + loop).onclick == null) {
	...
	for (loop = 0; loop < evaluationList.length; loop++) {
		if (document.getElementById('viewEvaluation' + loop) != null && document.getElementById('viewEvaluation' + loop).onclick == null) {
...
/*
 * helper: print evaluation actions:
 * 1. view
 * 2. edit
 * 3. remind
 * 4. delete
...
function printEvaluationActions(evaluationList, position){
	...
	output = output +
					...
					evaluationList[position].courseID + "','" + evaluationList[position].name +
					"');hideddrivetip();\"" +
					"onmouseover=\"ddrivetip('Delete the evaluation')\"" +
					"onmouseout=\"hideddrivetip()\">Delete</a>";
	//4.REMIND:
	output = output +
					"<a class='t_eval_remind' name=\"remindEvaluation" + position + "\" id=\"remindEvaluation" + position + "\"  href=\"javascript:toggleRemindStudents('" +
					evaluationList[position].courseID + "','" + evaluationList[position].name +
					"');hideddrivetip();\"" +
	...
	if(hasUnpublish){
		output = output +
					"<a class='t_eval_unpublish' name=\"publishEvaluation" + position + "\" id=\"publishEvaluation" + position + "\"  href=\"javascript:togglePublishEvaluation('" +
					evaluationList[position].courseID + "','" + evaluationList[position].name +
		...
		output = output +
					"<a class='t_eval_publish' name=\"unpublishEvaluation" + position + "\" id=\"publishEvaluation" + position + "\"  href=\"javascript:togglePublishEvaluation('" +
					evaluationList[position].courseID + "','" + evaluationList[position].name +
...
/*
 * UI Element: print evaluation summary form
...
function printEvaluationSummaryForm(submissionList, summaryList, status, commentsEnabled, type){
	...
	if(type == REVIEWER){
		...
		output = output +
					...
					"<th class=\"leftalign\"><input class=\"buttonSortNone\" type=\"button\" id=\"button_sortteamname\">TEAM</input></th>" +
					"<th class=\"leftalign\"><input class=\"buttonSortNone\" type=\"button\" id=\"button_sortname\">STUDENT</input></th>" +
					"<th class=\"leftalign\"><div  onmouseover=\"ddrivetip('" + HOVER_MESSAGE_CLAIMED + "')\" onmouseout=\"hideddrivetip()\"><input class=\"buttonSortNone\" type=\"button\" id=\"button_sortaverage\">CLAIMED CONTRIBUTION</input></div></th>" + 
					"<th class=\"centeralign\"><div  onmouseover=\"ddrivetip('" + HOVER_MESSAGE_PERCEIVED_CLAIMED + "')\" onmouseout=\"hideddrivetip()\"><input class=\"buttonSortNone\" type=\"button\" id=\"button_sortdiff\">[PERCEIVED - CLAIMED]</input></div></th>" +
					"<th class=\"centeralign\">ACTION(S)</th>" +
					"</tr>";
	...
	for (loop = 0; loop < summaryList.length; loop++) {
		...
		if(type == REVIEWER){
			...
			output = output +
						"<td>Equal Share" + displayEvaluationPoints(summaryList[loop].claimedPoints) + "</td>";
	...
	for (loop = 0; loop < summaryList.length; loop++) {
		if (document.getElementById('viewEvaluationResults' + loop) != null) {
			document.getElementById('viewEvaluationResults' + loop).onclick = function()  { 
				hideddrivetip();
				printEvaluationIndividualForm(submissionList, summaryList, this.id.substring(21, this.id.length), commentsEnabled, status, type);
				clearStatusMessage();
			};
		}
	}
...
/*
 * UI Element: print evaluation detail form
 * type: reviewer, reviewee
 * TODO: reviewer view show original or normalized points?
 * */
function printEvaluationDetailForm(submissionList, summaryList, status, commentsEnabled, type){
	clearStatusMessage();
//	//Sort by team first
//	summaryList = summaryList.sort(sortByTeamName);
	
	var output = (type == REVIEWER)? helpPrintTitle(REVIEWER_TITLE_DETAIL): helpPrintTitle(REVIEWEE_TITLE_DETAIL);
	...
	for (x = 0; x < summaryList.length; x++) {
		...
		output = output + helpPrintSubmission(submissionList, summaryList, x, status, commentsEnabled, type) + "</table>";
	}
...
/*
 * UI Element: print evaluation submission (individual)
 * type: Reviewer, Reviewee
 * */
function printEvaluationIndividualForm(submissionList, summaryList, position, commentsEnabled, status, type){
	var points;
	var justification = "";
	var commentsToStudent = "";
	var student = "";//used to show reviewer or reviewee contents
	var toStudent = summaryList[position].toStudent;
	var output = (type == REVIEWER)? helpPrintTitle(REVIEWER_TITLE_INDIVIDUAL): helpPrintTitle(REVIEWEE_TITLE_INDIVIDUAL);
	//Team name:
	output = output + helpPrintResultTeam(summaryList[position].teamName);
	//points:
	output = output + helpPrintResultHeader(type, summaryList[position].toStudentName, displayEvaluationPoints(summaryList[position].claimedPoints), displayEvaluationPoints(summaryList[position].average));
	...
	{	
		if((type == REVIEWEE && submissionList[loop].toStudent == toStudent) || (type == REVIEWER && submissionList[loop].fromStudent == toStudent))
		{
			// Extract data
			points = helpPrintPoints(submissionList[loop]);
			justification = helpPrintJustification(submissionList[loop]);
			commentsToStudent = helpPrintComments(submissionList[loop], commentsEnabled);
			
			// Print data
			if (submissionList[loop].fromStudent == submissionList[loop].toStudent) {
				outputTemp = helpPrintResultSelfComments(justification, commentsToStudent) + outputTemp;

			} else {
				student = (type == REVIEWEE)? submissionList[loop].fromStudentName : submissionList[loop].toStudentName;
				outputTemp = outputTemp + helpPrintResultOtherComments(student, points, justification, commentsToStudent);
			}
	...
	//buttons:
	output = output +  outputTemp + "</table></div>" + "<br /><br />" + 
	...
	if(type == REVIEWER){
		if (status == "CLOSED") {
			output = output + " <input type=\"button\" class =\"button\" type=\"button\" value=\"Edit\" name=\"button_edit\" id=\"button_edit\">";
		} else {
			output = output + " ";
		}
	}	
	
	output = output + " <input type=\"button\" class=\"button\" value=\"Back\" name=\"button_back\" id=\"button_back\"><br /><br />";
	...
	document.getElementById('button_next').onclick = function() { 
		...
		printEvaluationIndividualForm(submissionList, summaryList, position, commentsEnabled, status, type);
	};
...
function printEditEvaluationResultsByReviewer(submissionList, summaryList, position, commentsEnabled, status)
{
	...
	output = 
		"<form name=\"form_submitevaluation\" id=\"form_submitevaluation\">" + 
			"<p class=\"splinfo2\">TEAM: " + summaryList[position].teamName + "</p><br /><br />" +
			"<table class=\"headerform\">" + 
				"<tr style=\"display:none\">" +
	                "<td>" +
		                "<input type=\"text\" value=\"" + fromStudent + "\" name=\"" + STUDENT_FROMSTUDENT + 0 + "\" id=\"" + STUDENT_FROMSTUDENT + 0 + "\">" +
		                "<input type=\"text\" value=\"" + fromStudent + "\" name=\"" + STUDENT_TOSTUDENT + 0 + "\" id=\"" + STUDENT_TOSTUDENT + 0 + "\">" +
	                "</td>" +
		        "</tr>" +
		        "<tr style=\"display:none\">" +
		            "<td>" +
		                "<input type=\"text\" value=\"" + summaryList[position].teamName + "\" name=\"" + STUDENT_TEAMNAME + 0 + "\" id=\"" + STUDENT_TEAMNAME + 0 + "\">" +
		                "<input type=\"text\" value=\"" + submissionList[0].courseID + "\" name=\"" + COURSE_ID + 0 + "\" id=\"" + COURSE_ID + 0 + "\">" +
		            "</td>" +
		        "</tr>" +
		        "<tr style=\"display:none\">" +
		            "<td>" +
		                "<input type=\"text\" value=\"" + submissionList[0].evaluationName + "\" name=\"" + EVALUATION_NAME + 0 + "\" id=\"" + EVALUATION_NAME + 0 + "\">" +
		            "</td>" +
		        "</tr>" +

				"<tr>" + 
					"<td colspan=\"2\" class=\"reportheader\">" + summaryList[position].toStudentName.toUpperCase() + "'s Evaluation Submission</td>" +
				"</tr>" +
				"<tr>" +
					"<td class=\"lhs\">" +
		...
		if(submissionList[loop].fromStudent == fromStudent)
		{
			...
			if(submissionList[loop].fromStudent == submissionList[loop].toStudent)
			{
				if(commentsToStudent != "Disabled")
				{
					...
					"<tr>" +
						...
						"<td>" + 
							...
							"\" id=\"" + STUDENT_JUSTIFICATION + 0 + "\">" + sanitize(justification) + "</textarea>" +
						"</td>" +
					"</tr>" +
					"<tr>" +
				...
				else
				{
					...
					"<tr>" +
						"<td class=\"lhs\">" + "Comments about team dynamics:" + "</td>" +
						"<td>" +
							"<textarea class=\"textvalue\" rows=\"2\" cols=\"100\" name=\"" + STUDENT_COMMENTSTOSTUDENT + 0 +
							"\" id=\"" + STUDENT_COMMENTSTOSTUDENT + 0 + "\" disabled=\"disabled\">" +
								"Disabled" +
							"</textarea>" +
						"</td>" +
					"</tr>" + outputTemp;
			...
			else
			{
				if(commentsToStudent != "Disabled")
				{
		            ...
		            "<tr style=\"display:none\">" +
		                "<td>" +
		                    "<input type=\"text\" value=\"" + summaryList[position].teamName + "\" name=\"" + STUDENT_TEAMNAME + counter +
		                    "\" id=\"" + STUDENT_TEAMNAME + counter + "\">" +
		                    "<input type=\"text\" value=\"" + submissionList[0].courseID + "\" name=\"" + COURSE_ID + counter +
		                    "\" id=\"" + COURSE_ID + counter + "\">" +
		                "</td>" +
		            "</tr>" +
		            "<tr style=\"display:none\">" +
		                "<td>" +
		                    "<input type=\"text\" value=\"" + submissionList[0].evaluationName + "\" name=\"" + EVALUATION_NAME + counter +
		                    "\" id=\"" + EVALUATION_NAME + counter + "\">" +
		                "</td>" +
		            "</tr>" +
					"<tr>" +
						"<td colspan=\"2\" class=\"reportheader\">Evaluation To " + submissionList[loop].toStudentName.toUpperCase() + "</td>" +
					"</tr>" +
					"<tr>" +
						"<td class=\"lhs\">" +
							"Estimated contribution:" +
				...
				else
				{
					...
					"<tr>" +
						...
						"</td>" +
						"<td>" +
							"<textarea class=\"textvalue\" rows=\"2\" cols=\"100\" name=\"" + STUDENT_COMMENTSTOSTUDENT + counter +
							"\" id=\"" + STUDENT_COMMENTSTOSTUDENT + counter + "\" disabled=\"disabled\">" +
								"Disabled" +
							"</textarea>" +
						"</td>" +
					"</tr>";
	...
	"<form method=\"post\" action=\"\" name=\"form_joincourse\">" +
		"<table class=\"headerform\">" +
			"<tr>" +
				...
				"<td width=\"30%\">" +
					"<input class=\"keyvalue\" type=\"text\" name=\"" + STUDENT_REGKEY + "\" id=\"" + STUDENT_REGKEY + "\"" +
					"onmouseover=\"ddrivetip('Enter your registration key for the course.')\"" +
					"onmouseout=\"hideddrivetip()\" tabindex=1 />" +
				"</td>" +
				"<td width=\"30%\">" +
					"<input id='btnJoinCourse' type=\"button\" class=\"button\" onclick=\"doJoinCourse(this.form." + STUDENT_REGKEY + ".value);\" value=\"Join Course\" tabindex=2 />" +
				"</td>" +
			"</tr>" +
		"</table>" +
	"</form>";
...
function printCourseStudentForm(course)
{
	...
	var output = "<table width=\"600\" class=\"detailform\">" +
					 "<tr>" + 
						 "<td>Course ID:</td>" + 
 ...
 * User type: Student && Coordinator
 * */
function printEvaluationHeader(courseID, evaluationName, start, deadline, gracePeriod, instructions)
{
	var outputHeader = "<h1>EVALUATION RESULTS</h1>";
...
/*
 * Student do evaluation
 * Student edit evaluation submission
 * TODO: Improve UI
 * */
 ...
 * */
function printEvaluationResultStudentForm(summaryList, submissionList, start, deadline){
	...
	var output = 
		...
		"<table class=\"result_studentform\">" +
			"<tr>" +
				...
				"<td width=\"30%\">" + summaryList[0].toStudentName + "</td>" +
				"<td width=\"20%\">" + COURSE + "</td>" +
				"<td width=\"30%\">" + submissionList[0].courseID + "</td>" +
			"</tr>" +
			"<tr>" +
				"<td>" + TEAM + "</td>" +
				"<td>" + submissionList[0].teamName + "</td>" +
				"<td>" + EVALUATION + "</td>" +
				"<td>" + submissionList[0].evaluationName + "</td>" +
			"</tr>" +
			"<tr>" +
				"<td>" + CLAIMED + "</td>" +
				"<td>Equal Share" + displayEvaluationPoints(claimedPoints) + "</td>" +
				"<td>" + OPENING + "</td>" +
				"<td>" + start + "</td>" +
			"</tr>" +
			"<tr>" +
				"<td>" + PERCEIVED + "<br /><span class='color_gray'>" + ROUNDOFF + "</span></td>" +
				"<td>Equal Share" + displayEvaluationPoints(perceivedPoints) + "</td>" +
				"<td>" + CLOSING + "</td>" +
				"<td>" + deadline + "</td>" +
			"</tr>" +
		"</table>" +
	...
	for(loop = 0; loop < submissionList.length; loop++){
		...
		if(submissionList[loop].toStudent == targetStudent &&
			...
			if(sanitize(submissionList[loop].commentsToStudent) == ""){
				...
				output = output + "<tr><td>" + sanitize(submissionList[loop].commentsToStudent) + "</td></tr>";
			}
		}
	}
	
	output = output + "</tr></table><br /><br />" +
		 				"<input type=\"button\" class=\"button\" id=\"button_back\" onclick=\"displayEvaluationsTab();\" value=\"Back\" />" + "<br /><br />";
...
function helpPrintResultHeader(type, name, claimedPoints, perceivedPoints){
	var output =
		...
		"<thead>" +
		"<th colspan=\"2\" width=\"10%\"><span class=\"fontcolor\">" + type + ": </span>" + name + "</th>" +
		"<th><div onmouseover=\"ddrivetip('" + HOVER_MESSAGE_CLAIMED + "')\" onmouseout=\"hideddrivetip('')\"><span class=\"fontcolor\">" + CLAIMED +": </span>Equal Share" + claimedPoints + "</div></th>" +
		"<th><div onmouseover=\"ddrivetip('" + HOVER_MESSAGE_PERCEIVED + "')\" onmouseout=\"hideddrivetip('')\"><span class=\"fontcolor\">" + PERCEIVED +": </span>Equal Share" + perceivedPoints + "</div></th>" +
		"</thead>";
	return output;
}
function helpPrintResultSelfComments(justification, commentsToStudent){
	var output = 
		"<tr>" +
			"<td colspan=\"4\"><b>Self evaluation:</b><br />" + sanitize(justification) + "</td>" +
		 "</tr>" +
		 "<tr>" +
			"<td colspan=\"4\"><b>Comments about team:</b><br />" + sanitize(commentsToStudent) + "</td>" +
		 "</tr>";
	return output;
}
function helpPrintResultOtherComments(student, points, justification, commentsToStudent){
	var output = 
		"<tr>" +
			"<td><b>" + student + "</b></td>";
