...
3. Add remote names for committer repo (let's call it `upstream`)
   ```
...
4. Update your local repo (the one you created when setting up the project on your computer) 
   ...
   ```
5. If you have permissions to change labels, change the issue status to `s.Ongoing`. 

6. Start a new branch named `{IssueNumber}-{some-keywords}`. 
...
8. When the work is ready for review:
   ...
   * Ensure _dev green_ (i.e., all tests are passing on dev server).
    ...
    >If any of the browsertests fail, use [GodMode](https://docs.google.com/) to fix them. Run the test suite again without GodMode and ensure _dev green_
