...
9. When the work is ready for review:
   ...
   * <a name="creating-a-pull-request"></a>Create a pull request (PR). For the pull request name, copy paste
     ...
     Doing so will achieve an automatic reference (by simply mentioning the issue number with the #).<br>
     To close an issue from a PR, all you have to do is include the special keyword syntax (eg. "fixes #5") 
     in the body of your Pull Request. And the referenced issue will automatically be closed when the PR is merged into the default branch.<br>
