...
public class FieldValidator {
	...
	public String getInvalidStateInfo(FieldType fieldType, Object value) {
		...
		case INSTITUTE_NAME:
			return getInvalidStateInfo_INSTITUTE_NAME(value);
		default:
	...
	private String getInvalidStateInfo_INSTITUTE_NAME(Object value) {
		Assumption.assertTrue(value != null);
		String name = (String) value;
		if (name.trim().isEmpty()) {
			return String.format(INSTITUTE_NAME_ERROR_MESSAGE, name, REASON_EMPTY);
		}else if(name.trim().length()>INSTITUTE_NAME_MAX_LENGTH){
			return String.format(INSTITUTE_NAME_ERROR_MESSAGE, name, REASON_TOO_LONG);
		} else if(!name.trim().matches("[a-zA-Z0-9-,'/.&()\\s]*")){
			return String.format(INSTITUTE_NAME_ERROR_MESSAGE, name, REASON_DISALLOWED_CHAR);
		}
		return "";
	}
