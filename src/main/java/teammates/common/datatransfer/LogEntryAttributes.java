...
public class LogEntryAttributes extends EntityAttributes{
	private long time;
	 ...
	 */
	public LogEntryAttributes(String servlet, String params, String link){
		...
		email = "Unknown";
		toShow = true;
		message = "<span class=\"color_red\">Error. LogEntryAttributes object is not created for this servlet action.</span><br>"
				+ params;
		url = link;
	}
	 ...
	 */
	public LogEntryAttributes(AppLogLine appLog){
	 ...
	 */
	public LogEntryAttributes(String servlet, String act, AccountAttributes acc, String params,  String link){
		time = System.currentTimeMillis();
		servletName = servlet;
		action = act;
		toShow = true;
	...
	/**
	 * Constructor that creates an ActivityLog object from TeammatesLog fetched from datasource
	 * Used in the adminAcvityLog page
	 */
	public LogEntryAttributes(LogEntry log){
		time = System.currentTimeMillis();
		servletName = log.getServletName();
		...
		role = log.getRole();
		name = log.getName();
		googleId = log.getGoogleId();
		email = log.getEmail();
	}
	
	public LogEntryAttributes(AccountAttributes userAccount, boolean isMasquerade, String logMessage,  String requestUrl){
		time = System.currentTimeMillis();
	...
	public LogEntry toEntity(){
		return new LogEntry(servletName, action, toShow, role, name, googleId, email, message, url, time);
	}
	...
	public static String generateServletActionFailureLogMessage(HttpServletRequest req, Exception e){
        ...
        LogEntryAttributes exceptionLog = new LogEntryAttributes(action, Const.ACTION_RESULT_FAILURE, null, message, url);
	...
	public static String generateSystemErrorReportLogMessage(HttpServletRequest req, MimeMessage errorEmail) {
		...
		LogEntryAttributes emailReportLog = new LogEntryAttributes(action, Const.ACTION_RESULT_SYSTEM_ERROR_REPORT, null, message, url);
