...
public class InstructorFeedbackResultsSessionPanel {
    ...
    private String getResultsVisibleFromText(FeedbackSessionAttributes feedbackSession) {
        if (feedbackSession.getResultsVisibleFromTime().equals(Const.INSTANT_REPRESENTS_FOLLOW_VISIBLE)) {
            if (feedbackSession.getSessionVisibleFromTime().equals(Const.INSTANT_REPRESENTS_FOLLOW_OPENING)) {
                return TimeHelper.formatTime12H(feedbackSession.getStartTimeLocal());
            } else if (feedbackSession.getSessionVisibleFromTime().equals(Const.INSTANT_REPRESENTS_NEVER)) {
                return "Never";
            } else {
                return TimeHelper.formatTime12H(feedbackSession.getSessionVisibleFromTimeLocal());
            }
        } else if (feedbackSession.getResultsVisibleFromTime().equals(Const.INSTANT_REPRESENTS_LATER)) {
            return "I want to manually publish the results.";
        } else if (feedbackSession.getResultsVisibleFromTime().equals(Const.INSTANT_REPRESENTS_NEVER)) {
            return "Never";
        } else {
