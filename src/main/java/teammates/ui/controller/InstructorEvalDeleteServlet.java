...
public class InstructorEvalDeleteServlet extends ActionServlet<Helper> {
	...
	protected ActivityLogEntry instantiateActivityLogEntry(String servletName, String action, boolean toShows, Helper helper, String url, ArrayList<Object> data) {
		...
		if(action == Common.INSTRUCTOR_EVAL_DELETE_SERVLET_DELETE_EVALUATION){
			try {
				params = "Evaluation <span class=\"bold\">" + (String)data.get(1) + "</span> in Course <span class=\"bold\"[" + (String)data.get(0) + "]/span> deleted";
