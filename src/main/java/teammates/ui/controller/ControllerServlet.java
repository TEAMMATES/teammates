...
public class ControllerServlet extends HttpServlet {
	...
	public final void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			/*
			 * We are using the Template Method Design Pattern here. This method
			 * contains the high level logic of the the request processing.
			 * Concrete details of the processing steps are to be implemented by
			 * child classes, based on request-specific needs.
			 */
			...
			// This is the log message that is used to generate the 'activity
			// log' for the admin.
			log.info(c.getLogMessage());

		} catch (EntityDoesNotExistException e) {
			log.warning(LogEntryAttributes
					.generateServletActionFailureLogMessage(req, e));
			...
			// TODO: This exception is not caught because GAE kills the request
			// soon after throwing it.
			MimeMessage email = new Logic().emailErrorReport(
					req.getServletPath(),
					HttpRequestHelper.printRequestParameters(req),
					e);
			log.severe(LogEntryAttributes.generateSystemErrorReportLogMessage(
					req, email));
