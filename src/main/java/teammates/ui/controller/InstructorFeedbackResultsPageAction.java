...
public class InstructorFeedbackResultsPageAction extends Action {
	...
	protected ActionResult execute() throws EntityDoesNotExistException,
		...
		if(courseId==null || feedbackSessionName == null) {
			return createRedirectResult(Config.PAGE_INSTRUCTOR_HOME);
		}
		...
		data.sortType = getRequestParam(Config.PARAM_FEEDBACK_RESULTS_SORTTYPE);
		...
		if (data.sortType == null) {
			...
			return createShowPageResult(Config.JSP_INSTRUCTOR_FEEDBACK_RESULTS_BY_RECIPIENT, data);
		}
		if (data.sortType.equals("table")){
			statusToUser.add("Displaying feedback session results as a table.");
			return createShowPageResult(Config.JSP_INSTRUCTOR_FEEDBACK_RESULTS_BY_TABLE, data);
			...
			return createShowPageResult(Config.JSP_INSTRUCTOR_FEEDBACK_RESULTS_BY_GIVER, data);
		} else {
			// default: sort by recipients
			return createShowPageResult(Config.JSP_INSTRUCTOR_FEEDBACK_RESULTS_BY_RECIPIENT, data);
		}
