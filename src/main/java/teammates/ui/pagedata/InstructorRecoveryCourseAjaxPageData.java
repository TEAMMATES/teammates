...
public class InstructorRecoveryCourseAjaxPageData extends PageData {

    private CourseTable courseTable;
    private int index;

    public InstructorRecoveryCourseAjaxPageData(AccountAttributes account, String sessionToken) {
    ...
    private List<HomeFeedbackSessionRow> createSessionRows(List<FeedbackSessionAttributes> sessions,
        ...
        for (FeedbackSessionAttributes session : sessions) {
            InstructorHomeFeedbackSessionRow row = new InstructorHomeFeedbackSessionRow(
                    ...
                    TimeHelper.formatDateTimeForInstructorRecoveryPage(session.getStartTimeLocal()),
                    session.getStartTimeInIso8601UtcFormat(),
                    TimeHelper.formatDateTimeForDisplay(session.getStartTime(), session.getTimeZone()),
                    TimeHelper.formatDateTimeForInstructorRecoveryPage(session.getEndTimeLocal()),
                    ...
                    getInstructorFeedbackSessionActions(
                            session, Const.ActionURIs.INSTRUCTOR_RECOVERY_PAGE, instructor));
