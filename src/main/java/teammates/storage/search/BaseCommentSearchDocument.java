...
public abstract class BaseCommentSearchDocument extends SearchDocument {
    ...
    protected StringBuilder prepareCommonSearchableTextBuilder(
            BaseCommentAttributes comment, String delim) {
        StringBuilder searchableTextBuilder = new StringBuilder("");
        searchableTextBuilder.append(comment.courseId).append(delim);
        searchableTextBuilder.append(course != null ? course.name : "").append(
                delim);
        searchableTextBuilder.append(comment.giverEmail).append(delim);
        searchableTextBuilder.append(
                giverAsInstructor != null ? giverAsInstructor.name : "")
                .append(delim);
        searchableTextBuilder.append(comment.commentText.getValue()).append(delim);
        return searchableTextBuilder;
    }

    protected Builder prepareCommonDocumentPart(BaseCommentAttributes comment,
            String giverEmail, StringBuilder searchableTextBuilder, String instructor,
            String commentAttrField, String commentGiverNameField) {
        Builder docBuilder = Document.newBuilder().setId(comment.getId().toString());
        docBuilder.addField(Field.newBuilder().setName(Const.SearchDocumentField.COURSE_ID).setText(comment.courseId))
            .addField(Field.newBuilder().setName(Const.SearchDocumentField.GIVER_EMAIL).setText(giverEmail))
            ...
            .addField(Field.newBuilder().setName(Const.SearchDocumentField.IS_VISIBLE_TO_INSTRUCTOR).setText(instructor))
            // attribute field is used to convert a doc back to attribute
            .addField(Field.newBuilder().setName(commentAttrField).setText(new Gson().toJson(comment)))
            .addField(Field.newBuilder().setName(commentGiverNameField).setText(
