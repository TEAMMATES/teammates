...
function checkFeedbackQuestion(form) {
    ...
    if (recipientType === 'STUDENTS' || recipientType === 'TEAMS') {
        if ($(form).find('[name|=' + FEEDBACK_QUESTION_NUMBEROFENTITIESTYPE + ']:checked')
            ...
            setStatusMessage(DISPLAY_FEEDBACK_QUESTION_NUMBEROFENTITIESINVALID, STATUS_DANGER);
            return false;
        }
    }
    if (!$(form).find('[name=' + FEEDBACK_QUESTION_TEXT + ']').val()) {
        setStatusMessage(DISPLAY_FEEDBACK_QUESTION_TEXTINVALID, STATUS_DANGER);
        return false;
    }
    if ($(form).find('[name=' + FEEDBACK_QUESTION_TYPE + ']').val() === 'NUMSCALE') {
        if (!$(form).find('[name=' + FEEDBACK_QUESTION_NUMSCALE_MIN + ']').val() ||
            ...
            setStatusMessage(DISPLAY_FEEDBACK_QUESTION_NUMSCALE_OPTIONSINVALID, STATUS_DANGER);
            return false;
        }
        ...
        if (updateNumScalePossibleValues(qnNum)) {
            ...
            setStatusMessage(DISPLAY_FEEDBACK_QUESTION_NUMSCALE_INTERVALINVALID, STATUS_DANGER);
            return false;
        }
    }
    return true;
...
function checkEditFeedbackSession(form) {
    if (form.visibledate.getAttribute('disabled')) {
        if (!form.visibledate.value) {
            setStatusMessage(DISPLAY_FEEDBACK_SESSION_VISIBLE_DATEINVALID, STATUS_DANGER);
            return false;
        }
    }
    if (form.publishdate.getAttribute('disabled')) {
        if (!form.publishdate.value) {
            setStatusMessage(DISPLAY_FEEDBACK_SESSION_PUBLISH_DATEINVALID, STATUS_DANGER);
            return false;
        }
    }
...
function bindCopyButton() {
    $('#button_copy').on('click', function(e) {
        ...
        if (!$sessionsList.length) {
            setStatusMessage(FEEDBACK_SESSION_COPY_INVALID, STATUS_DANGER);
            return false;
        }
        ...
        if (isExistingSession) {
            setStatusMessage(DISPLAY_FEEDBACK_SESSION_NAME_DUPLICATE, STATUS_DANGER);
        } else {
            clearStatusMessages();
