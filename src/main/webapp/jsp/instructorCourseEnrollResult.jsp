...
<body>
    ...
    <div class="container" id="frameBodyWrapper">
        ...
        <%  for (int i = 0; i < StudentAttributes.UpdateStatus.STATUS_COUNT; i++) {
            ...
            <%  if (students.size() > 0) { %>  
                <div 
                    <%  StudentAttributes.UpdateStatus status = StudentAttributes.UpdateStatus.enumRepresentation(i); 
                        if (status == StudentAttributes.UpdateStatus.ERROR) { %>
                            class="panel panel-danger"
                    <%  } else if (status == StudentAttributes.UpdateStatus.NEW) { %>
                            class="panel panel-primary"
                    <%  } else if (status == StudentAttributes.UpdateStatus.MODIFIED) { %>
                            class="panel panel-warning"
                    <%  } else if (status == StudentAttributes.UpdateStatus.UNMODIFIED) { %>
                            class="panel panel-info"
                    <%  } else if (status == StudentAttributes.UpdateStatus.NOT_IN_ENROLL_LIST) { %>
                            class="panel panel-default"
                    <%  } else if (status == StudentAttributes.UpdateStatus.UNKNOWN) { %>
                            class="panel panel-danger"
                    <%  } else { %>
                            class="panel panel-danger"
                    <%  } %>
