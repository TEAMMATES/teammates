			...
			<form method="post" action="<%=Const.ActionURIs.INSTRUCTOR_FEEDBACK_ADD%>" name="form_addfeedbacksession">
				...
				<table class="inputTable sessionTable" id="sessionViewableTable">
					<tr>
						...
						<td onmouseover="ddrivetip('<%=Const.Tooltips.FEEDBACK_SESSION_VISIBLEDATE%>')"
							...
							<%if(data.newFeedbackSession!=null &&
									 data.newFeedbackSession.sessionVisibleFromTime.equals(Const.TIME_REPRESENTS_FOLLOW_OPENING) == false &&
									 data.newFeedbackSession.sessionVisibleFromTime.equals(Const.TIME_REPRESENTS_LATER) == false)  
							...
							disabled="if(document.getElementById('<%=Const.ParamsNames.FEEDBACK_SESSION_SESSIONVISIBLEBUTTON%>_custom').checked){'disabled'}else{''}"
							> @ <select
							style="width: 70px;"
						...
						<td onmouseover="ddrivetip('<%=Const.Tooltips.FEEDBACK_SESSION_SESSIONVISIBLEATOPEN%>')"
							...
							<%if(data.newFeedbackSession==null || data.newFeedbackSession.sessionVisibleFromTime.equals(Const.TIME_REPRESENTS_FOLLOW_OPENING)) 
									out.print("checked=\"checked\"");%>
						...
						<td colspan="2" onmouseover="ddrivetip('<%=Const.Tooltips.FEEDBACK_SESSION_SESSIONVISIBLENEVER%>')"
							...
							<%if(data.newFeedbackSession!=null && data.newFeedbackSession.sessionVisibleFromTime.equals(Const.TIME_REPRESENTS_NEVER)) 
									out.print("checked=\"checked\"");%>							
							onclick="document.getElementById('<%=Const.ParamsNames.FEEDBACK_SESSION_VISIBLEDATE%>').disabled='disabled'
							document.getElementById('<%=Const.ParamsNames.FEEDBACK_SESSION_VISIBLETIME%>').disabled='disabled';">
				...
				<table class="inputTable sessionTable" id="timeFrameTable">
					<tr>
						...
						<td onmouseover="ddrivetip('<%=Const.Tooltips.FEEDBACK_SESSION_ENDDATE%>')"
							onmouseout="hideddrivetip()">
							<input style="width: 100px;" type="text"
