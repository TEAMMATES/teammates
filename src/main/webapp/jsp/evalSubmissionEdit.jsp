...
<% EvalSubmissionEditHelper helper = (EvalSubmissionEditHelper)request.getAttribute("helper"); %>
					...
					<input type="hidden" value="<%= EvalSubmissionEditHelper.escapeHTML(helper.eval.name) %>"
							name="<%= Common.PARAM_EVALUATION_NAME %>"
							id="<%= Common.PARAM_EVALUATION_NAME %>" />
					<input type="hidden" value="<%= EvalSubmissionEditHelper.escapeHTML(helper.student.team) %>"
		 					...
		 					<tr>
		 						...
		 						<td>
		 							<textarea class="textvalue" rows="8" cols="100" 
		 									...
		 									id="<%= Common.PARAM_JUSTIFICATION+idx %>"><%= EvalSubmissionEditHelper.escapeHTML(sub.justification.getValue()) %></textarea>
		 						</td>
		 					</tr>
		 					<tr>
								...
								<%	if(helper.eval.p2pEnabled){ %>
									<td><textarea class = "textvalue"
											...
											name="<%= Common.PARAM_COMMENTS %>"
									 		id="<%= Common.PARAM_COMMENTS+idx %>"><%= EvalSubmissionEditHelper.escapeHTML(sub.p2pFeedback.getValue()) %></textarea>
									</td>
								<%	} else { %>
									<td>
										<font color="red">
