        ...
        <div class="well well-plain">
            ...
            <c:if test="${not empty data.comments}"> <%--check student comments starts--%> 
                ...
                <div id="panel_display-${panelIdx}">
                    <br>
                    <%-- <comments:commentsForStudentsPanel /> --%>
                </div>
            </c:if> <%--check student comments ends --%>
            <c:set var="fsIdx" value="0" scope="page" />
            <c:forEach items="${data.feedbackSessions}" var="fs"> <%--FeedbackSession loop starts--%>
                <c:set var="fsName" value="${fs.sessionName}" scope="page" />
                <c:set var="fsIdx" value="${fsIdx + 1}" scope="page" />
                <c:set var="panelIdx" value="${panelIdx + 1}" scope="page" />
                <div id="panel_display-${panelIdx}">
                    <br>
                    <comments:commentsFromFeedbackSessions fsName="${fsName}" fsIdx="${fsIdx}" panelIdx="${panelIdx}" />
                </div>
