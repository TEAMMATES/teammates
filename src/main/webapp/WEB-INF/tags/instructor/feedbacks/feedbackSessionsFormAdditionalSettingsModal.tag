...
<%@ attribute name="settingOptions" type="java.util.Collection" required="true"%>
<%@ attribute name="settingOptionsDescription" type="java.util.Collection" required="true"%>
  ...
  <div>
    <ul>
      <c:forEach items="${settingOptions}" var="option" varStatus="loop">
        <li><p><label>${option}:</label> ${settingOptionsDescription.get(loop.index)}</p></li>
      </c:forEach>
    </ul>
  </div>
</div>
