...
<form class="inline" method="post" action="${data.instructorEditStudentFeedbackLink}" target="_blank"> 
    <input type="submit" class="btn btn-default btn-xs" value="Moderate Response" <c:if test="${moderationButton.isDisabled}">disabled="disabled"</c:if> data-toggle="tooltip" title="<%=Const.Tooltips.FEEDBACK_SESSION_MODERATE_FEEDBACK%>">
