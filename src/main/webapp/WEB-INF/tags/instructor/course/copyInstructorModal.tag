<%@ tag description="instructorCourseEdit - course 'copy instructor' modal" pageEncoding="UTF-8" %>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="copyModalTitle">Copy Instructors</h4>
      </div>
      <div class="modal-body">
        <form class="form" id="copyModalForm" role="form" method="post"
              action="<%= Const.ActionURIs.INSTRUCTOR_COURSE_INSTRUCTOR_COPY %>">

          <input type="hidden" name="<%= Const.ParamsNames.USER_ID %>" value="${data.account.googleId}">
          <input type="hidden" name="<%= Const.ParamsNames.COURSE_ID %>" value="${courseId}">
          <input type="hidden" name="<%= Const.ParamsNames.SESSION_TOKEN %>" value="${data.sessionToken}">
        </form>
        <div id="instructor-copy-modal-status"></div>
      </div>
