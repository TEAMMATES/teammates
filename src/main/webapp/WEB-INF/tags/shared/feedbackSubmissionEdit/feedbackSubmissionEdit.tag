...
<c:choose>
  <c:when test="${isInstructor}">
    <ti:instructorPage pageTitle="Submit Feedback [Instructor]" bodyTitle="Submit Feedback" jsIncludes="${jsIncludes}" altNavBar="${altHeader}">
      <tsfse:feedbackSubmissionForm moderatedPersonEmail="${moderatedPersonEmail}"/>
    </ti:instructorPage>
  </c:when>
  <c:otherwise>
    <ts:studentPage pageTitle="Submit Feedback [Student]" bodyTitle="Submit Feedback" jsIncludes="${jsIncludes}" altNavBar="${altHeader}">
