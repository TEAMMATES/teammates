...
<c:choose>
  <c:when test="${isInstructor}">
    <ti:instructorPage pageTitle="Submit Feedback" bodyTitle="Submit Feedback" jsIncludes="${jsIncludes}" altNavBar="${altHeader}">
      <tsfse:feedbackSubmissionForm moderatedPersonEmail="${moderatedPersonEmail}"/>
    </ti:instructorPage>
  </c:when>
  <c:otherwise>
    <ts:studentPage pageTitle="Submit Feedback" bodyTitle="Submit Feedback" jsIncludes="${jsIncludes}" altNavBar="${altHeader}">
