...
public class InstructorFeedbackSessionsPageSql extends AppPage {
    ...
    public void verifySessionsTable(FeedbackSession[] sessions) {
    ...
    public void addCopyOfSession(FeedbackSession sessionToCopy, Course copyToCourse,
        ...
        selectSessionToCopy(sessionToCopy.getCourseId(), sessionToCopy.getName());
    ...
    public void copySession(FeedbackSession sessionToCopy, Course copyToCourse,
                            String newSessionName) {
        String copyFromCourse = sessionToCopy.getCourseId();
        String sessionName = sessionToCopy.getName();
    ...
    public void publishSessionResults(FeedbackSession sessionToPublish) {
        int rowId = getFeedbackSessionRowId(sessionToPublish.getCourseId(), sessionToPublish.getName());
        click(browser.driver.findElement(By.className("btn-results-" + rowId)));
        clickAndConfirm(waitForElementPresence(By.className("btn-publish-" + rowId)));
    }

    public void unpublishSessionResults(FeedbackSession sessionToPublish) {
        int rowId = getFeedbackSessionRowId(sessionToPublish.getCourseId(), sessionToPublish.getName());
        click(browser.driver.findElement(By.className("btn-results-" + rowId)));
        clickAndConfirm(waitForElementPresence(By.className("btn-unpublish-" + rowId)));
    }

    public void downloadResults(FeedbackSession session) {
        int rowId = getFeedbackSessionRowId(session.getCourseId(), session.getName());
        click(browser.driver.findElement(By.className("btn-results-" + rowId)));
    ...
    private String[] getSoftDeletedSessionDetails(FeedbackSession session) {
        String[] details = new String[4];
        details[0] = session.getCourseId();
        details[1] = session.getName();
        details[2] = getSimpleDateString(session.getCreatedAt(), session.getCourse().getTimeZone());
        details[3] = getSimpleDateString(session.getDeletedAt(), session.getCourse().getTimeZone());
        return details;
    }
    ...
    private void setResponseVisibilitySettings(FeedbackSession newFeedbackSession) {
        ...
        if (responseDateTime.equals(Const.TIME_REPRESENTS_FOLLOW_VISIBLE)) {
            ...
            setResponseDateTime(responseDateTime, newFeedbackSession.getCourse().getTimeZone());
        }
    }
