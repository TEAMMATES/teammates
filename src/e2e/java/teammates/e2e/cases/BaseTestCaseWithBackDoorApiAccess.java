...
/**
 * Base class for all test cases which are allowed to access the Datastore via
 * {@link BackDoor}.
 */
...
public abstract class BaseTestCaseWithBackDoorApiAccess extends BaseTestCaseWithDatastoreAccess {
	...
	protected AccountAttributes getAccountWithRetry(String googleId) throws MaximumRetriesExceededException {
		return getPersistenceRetryManager()
				.runUntilNotNull(new RetryableTaskReturns<AccountAttributes>(Const.TestCase.GET_ACCOUNT) {
					@Override
	...
	protected CourseAttributes getCourseWithRetry(String courseId) throws MaximumRetriesExceededException {
		return getPersistenceRetryManager()
				.runUntilNotNull(new RetryableTaskReturns<CourseAttributes>(Const.TestCase.GET_COURSE) {
					@Override
	...
	protected FeedbackQuestionAttributes getFeedbackQuestion(String courseId, String feedbackSessionName,
			int qnNumber) {
	...
	protected FeedbackQuestionAttributes getFeedbackQuestionWithRetry(String courseId, String feedbackSessionName,
			int qnNumber) throws MaximumRetriesExceededException {
		return getPersistenceRetryManager().runUntilNotNull(
				new RetryableTaskReturns<FeedbackQuestionAttributes>(Const.TestCase.GET_FEEDBACK_QUESTION) {
					@Override
	...
	protected FeedbackResponseAttributes getFeedbackResponse(String feedbackQuestionId, String giver,
			String recipient) {
	...
	protected FeedbackSessionAttributes getFeedbackSessionWithRetry(String courseId, String feedbackSessionName)
		...
		return getPersistenceRetryManager().runUntilNotNull(
				new RetryableTaskReturns<FeedbackSessionAttributes>(Const.TestCase.GET_FEEDBACK_SESSION) {
					@Override
	...
	protected InstructorAttributes getInstructorWithRetry(String courseId, String instructorEmail)
			throws MaximumRetriesExceededException {
		return getPersistenceRetryManager()
				.runUntilNotNull(new RetryableTaskReturns<InstructorAttributes>(Const.TestCase.GET_INSTRUCTOR) {
					@Override
	...
	protected String getKeyForInstructorWithRetry(String courseId, String instructorEmail)
			throws MaximumRetriesExceededException {
		return getPersistenceRetryManager()
				.runUntilSuccessful(new RetryableTaskReturns<String>(Const.TestCase.GET_KEY_FOR_INSTRUCTOR) {
					@Override
					public String run() {
