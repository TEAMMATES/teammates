...
public class PatchCreatedAtTimeNotification extends DatastoreClient {
    ...
    protected void migrateEntity(teammates.storage.entity.Notification oldEntity) {
        ...
        CriteriaQuery<teammates.storage.sqlentity.Notification> cr = cb.createQuery(teammates.storage.sqlentity.Notification.class);
        Root<teammates.storage.sqlentity.Notification> root = cr.from(teammates.storage.sqlentity.Notification.class);

        cr.select(root).where(cb.equal(root.get("title"), oldEntity.getTitle()));
        List<teammates.storage.sqlentity.Notification> matchingNotifs = HibernateUtil.createQuery(cr).getResultList();
        if (matchingNotifs.size() > 1) {
            throw new Error("More than one matching notification found with title");
        } else if (matchingNotifs.size() == 0){
            throw new Error("No notification found with title");
        }

        // Get first items since there is guaranteed to be one
        teammates.storage.sqlentity.Notification newNotif = matchingNotifs.get(0);
