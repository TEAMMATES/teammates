...
public class FeedbackNumericalScaleQuestionDetailsTest extends BaseTestCase {
    ...
    public void tesValidateResponseDetails() {
        ...
        errors = numScaleQuestion.validateResponsesDetails(Arrays.asList(resp), 1);
        assertTrue(errors.size() == 1);
        assertEquals("5.1 is out of the range for Numerical-scale question.(min=1, max=5)", errors.get(0));

        ______TS("Test Val=0 and Max=5,Step=1 triggers error");
        resp = new FeedbackNumericalScaleResponseDetails();
        resp.setAnswer(5.1);
        errors = numScaleQuestion.validateResponsesDetails(Arrays.asList(resp), 1);
        assertTrue(errors.size() == 1);
        ...
        assertEquals("Please enter a valid value. The two nearest valid values are 1.0 and 2.0.", errors.get(0));
        assertEquals("100 is out of the range for Numerical-scale question.(min=1, max=5)", errors.get(1));
    }
