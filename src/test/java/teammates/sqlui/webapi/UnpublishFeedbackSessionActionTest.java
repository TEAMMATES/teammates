...
public class UnpublishFeedbackSessionActionTest extends BaseActionTest<UnpublishFeedbackSessionAction> {
    ...
    void setUp() throws InvalidParametersException, EntityDoesNotExistException {
        ...
        when(mockLogic.unpublishFeedbackSession(
    ...
    protected void testExecute() {
        ...
        UnpublishFeedbackSessionAction unpublishFeedbackSessionAction = getAction(params);

        JsonResult result = getJsonResult(unpublishFeedbackSessionAction);
        ...
        assertEquals(FeedbackSessionPublishStatus.NOT_PUBLISHED, feedbackSessionData.getPublishStatus());
        assertFalse(mockLogic.getFeedbackSession(feedbackSession1.getName(), course1.getId()).isPublished());

        ______TS("Typical case: Session is already unpublished");
    ...
    private FeedbackSession generateSession2InCourse(Course course) {
        FeedbackSession fs = new FeedbackSession("feedbacksession-1", course,
                ...
                Instant.parse("2012-03-28T22:00:00Z"), Const.TIME_REPRESENTS_LATER,
                Duration.ofHours(10), true, true, true);
        fs.setCreatedAt(Instant.parse("2023-01-01T00:00:00Z"));
        fs.setUpdatedAt(Instant.parse("2023-01-01T00:00:00Z"));
