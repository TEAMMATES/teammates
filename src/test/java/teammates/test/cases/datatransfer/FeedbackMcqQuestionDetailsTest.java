...
public class FeedbackMcqQuestionDetailsTest extends BaseTestCase {
    ...
    public void testGetMcqOtherWeight_shouldChangesRequireResponseDeletionTest_shouldReturnFalse() {
        ...
        requestParams.put(Const.ParamsNames.FEEDBACK_QUESTION_MCQCHOICE + "-1", new String[] { "Choice 2" });
        requestParams.put(Const.ParamsNames.FEEDBACK_QUESTION_MCQ_HAS_WEIGHTS_ASSIGNED, new String[] { "on" });
        requestParams.put(Const.ParamsNames.FEEDBACK_QUESTION_MCQ_WEIGHT + "-0", new String[] { "1.23" });
        requestParams.put(Const.ParamsNames.FEEDBACK_QUESTION_MCQ_WEIGHT + "-1", new String[] { "2.22" });
        requestParams.put(Const.ParamsNames.FEEDBACK_QUESTION_MCQOTHEROPTIONFLAG, new String[] { "on" });
        requestParams.put(Const.ParamsNames.FEEDBACK_QUESTION_MCQ_OTHER_WEIGHT, new String[] { "4.17" });

        assertFalse(mcqDetails.hasAssignedWeights());
        assertEquals(0.0, mcqDetails.getMcqOtherWeight());
        assertFalse(mcqDetails.shouldChangesRequireResponseDeletion(mcqDetails)); //will always be false
        assertEquals("Feedback", mcqDetails.getCsvHeader());
    }
