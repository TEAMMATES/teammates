...
public class FeedbackResponseCommentSearchTest extends BaseSearchTest {
    ...
    public void allTests() throws InvalidParameterException {
        ...
        FeedbackResponseCommentAttributes fdbrI1S1 = dataBundle.feedbackResponseComments
                .get("comment1FromT1C1ToR1Q1S1C1");
        FeedbackResponseCommentAttributes fdbrI1S1Q2 = dataBundle.feedbackResponseComments
                .get("comment1FromT1C1ToR1Q2S1C1");
        FeedbackResponseCommentAttributes fdbrI1S1Q3 = dataBundle.feedbackResponseComments
        ...
        ______TS(
                "success: search for feedbackresponsecomments; query string does not match any "
                        + "feedbackresponsecomments; results restricted "
                        + "based on instructor's privilege");

        FeedbackResponseCommentSearchResultBundle bundle = feedbackResponseCommentsDb.search("non-existent",
                ins1OfCourse1);
        ...
        ______TS(
                "success: search for feedbackresponsecomments; query string should be case-insensitive;results restricted "
                        + "based on instructor's privilege");

        bundle = feedbackResponseCommentsDb
                .search("Instructor 1 COMMENT to student 1 self feedback Question 2", ins1OfCourse1);
        verifySearchResults(bundle, fdbrI1S1Q2, fdbrI1S1);

        ______TS(
                "success: search for feedbackresponsecomments by searchable by courseId; results restricted "
                        + "based on instructor's privilege");
    ...
    private static void verifySearchResults(FeedbackResponseCommentSearchResultBundle actual,
        ...
        for (List<FeedbackResponseCommentAttributes> feedbackResponseCommentAttributesList : actual.comments
                .values()) {
            standardizedFeedbackResponseCommentsForComparison(feedbackResponseCommentAttributesList
                    .toArray(
                            new FeedbackResponseCommentAttributes[feedbackResponseCommentAttributesList
                                                                  .size()]));
            feedbackCommentListNormalised.add(feedbackResponseCommentAttributesList.get(0));

        }

        AssertHelper.assertSameContentIgnoreOrder(Arrays.asList(expected),
                feedbackCommentListNormalised);
    }
