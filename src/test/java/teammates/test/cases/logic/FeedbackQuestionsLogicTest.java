...
public class FeedbackQuestionsLogicTest extends BaseLogicTest {
  ...
  private void testGetRecipientsForQuestion() throws Exception {
    ...
    email = dataBundle.students.get("student1InCourse1").email;
    AccountsLogic.inst()
        .makeAccountInstructor(dataBundle.students.get("student1InCourse1").googleId);
  ...
  private void testUpdateQuestionNumber() throws Exception {
    ...
    List<FeedbackQuestionAttributes> actualList =
        fqLogic.getFeedbackQuestionsForSession(questionToUpdate.feedbackSessionName,
                                               questionToUpdate.courseId);
    ...
    actualList = fqLogic.getFeedbackQuestionsForSession(questionToUpdate.feedbackSessionName,
                                                        questionToUpdate.courseId);
  ...
  private void testCopyQuestion() throws Exception {
    ...
    question1 = fqLogic.getFeedbackQuestion(question1.feedbackSessionName,
                                            question1.courseId,
                                            question1.questionNumber);

    FeedbackQuestionAttributes copiedQuestion =
        fqLogic.copyFeedbackQuestion(question1.getId(),
                                     question1.feedbackSessionName,
                                     question1.courseId,
    ...
    assertEquals(question1.numberOfEntitiesToGiveFeedbackTo,
                 copiedQuestion.numberOfEntitiesToGiveFeedbackTo);
  ...
  private void testGetFeedbackQuestionsForInstructor() throws Exception {
    ...
    actualQuestions =
        fqLogic.getFeedbackQuestionsForInstructor("Instructor feedback session",
                                                  "idOfTypicalCourse2",
    ...
    ______TS(
        "Get questions for creator instructor, verifies that the two methods return the same questions");
    expectedQuestions = new ArrayList<FeedbackQuestionAttributes>(actualQuestions);
    actualQuestions =
        fqLogic.getFeedbackQuestionsForCreatorInstructor("Instructor feedback session",
                                                         "idOfTypicalCourse2");
    ...
    try {
      fqLogic.getFeedbackQuestionsForInstructor("Instructor feedback session", "idOfTypicalCourse1",
                                                "instructor1@course1.tmt");
      signalFailureToDetectException(
          "Allowed to get questions for a feedback session that does not exist.");
    } catch (EntityDoesNotExistException e) {
  ...
  private void testIsQuestionAnswered() throws Exception {
    ...
    assertFalse(fqLogic.isQuestionFullyAnsweredByUser(question,
                                                      "studentWithNoResponses@gmail.tmt"));

    ______TS("test question is fully answered by team");

  }

  private void testAddQuestionNoIntegrityCheck()
    ...
    int initialNumQuestions = fqLogic.getFeedbackQuestionsForSession(q1.feedbackSessionName,
                                                                     q1.courseId).size();
