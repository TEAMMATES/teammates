...
import teammates.test.driver.RetryableTaskWithResultUnchecked;
...
public abstract class BaseTestCaseWithBackDoorApiAccess extends BaseTestCaseWithDatastoreAccess {
    ...
    protected CourseAttributes getCourseWithRetry(final String courseId) {
        return RetryManager.runWithRetry(new RetryableTaskWithResultUnchecked<CourseAttributes>("getCourse") {
            @Override
            public boolean run() {
    ...
    protected InstructorAttributes getInstructorWithRetry(final String courseId, final String instructorEmail) {
        return RetryManager.runWithRetry(new RetryableTaskWithResultUnchecked<InstructorAttributes>("getInstructor") {
            @Override
            public boolean run() {
    ...
    protected String getKeyForInstructorWithRetry(final String courseId, final String instructorEmail) {
        return RetryManager.runWithRetry(new RetryableTaskWithResultUnchecked<String>("getKeyForInstructor") {
            @Override
            public boolean run() {
