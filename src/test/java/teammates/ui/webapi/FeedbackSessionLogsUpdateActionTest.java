...
public class FeedbackSessionLogsUpdateActionTest
    ...
    public void testExecute() throws Exception {
        ...
        List<FeedbackSessionLogEntryAttributes> logs = logic.getFeedbackSessionLogs(
                ...
                currentTime.minus(1, ChronoUnit.SECONDS).toEpochMilli(),
                currentTime.toEpochMilli(), null);

        assertEquals(1, logs.size());

        FeedbackSessionLogEntryAttributes log = logs.get(0);

        assertEquals("student1InCourse1@gmail.tmt", log.getStudentEmail());
        assertEquals("First feedback session", log.getFeedbackSessionName());
        assertEquals(FeedbackSessionLogType.ACCESS.getLabel(), log.getFeedbackSessionLogType());

        // wait 5s to isolate the previous log window
        ThreadHelper.waitFor(5 * 1000);
        ...
        currentTime = Instant.now();
        logs = logic.getFeedbackSessionLogs("idOfTypicalCourse1", null,
                currentTime.minus(3, ChronoUnit.SECONDS).toEpochMilli(),
                currentTime.toEpochMilli(), null);
