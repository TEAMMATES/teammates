...
public class TestCoordRemindIndividualJoinCourse extends BaseTest {
	...
	public static void classSetup() {
		...
		try {
			...
			SharedLib.markAllEmailsSeen(Config.inst().INDIVIDUAL_ACCOUNT,
					Config.inst().TEAMMATES_APP_PASSWD);
		} catch (Exception e) {
	...
	public void testRemindIndividualJoinCourse() throws Exception {
		...
		assertEquals(key, SharedLib.getRegistrationKeyFromGmail(
				Config.inst().INDIVIDUAL_ACCOUNT, Config.inst().TEAMMATES_APP_PASSWD,
				sc.course.courseId));
		...
		assertEquals("", SharedLib.getRegistrationKeyFromGmail(
				sc.students.get(0).email, Config.inst().TEAMMATES_APP_PASSWD,
				sc.course.courseId));
