...
QUnit.test('toggleDeleteCourseConfirmation(courseID)', function(assert){
    // gives a popup, can't be tested
    expect(0);
});


QUnit.test('toggleDeleteEvaluationConfirmation(courseID, name)', function(assert){
    // N/A, displays popup
    expect(0);
});


QUnit.test('togglePublishEvaluation(name)', function(assert){
    // N/A, uses elements in the page
    expect(0);
});


QUnit.test('toggleUnpublishEvaluation(name)', function(assert){
    // N/A, uses elements in the page
    expect(0);
});


QUnit.test('toggleRemindStudents(evaluationName)', function(assert){
    // N/A, uses elements in the page
    expect(0);
});
...
QUnit.test('isStudentTeamNameValid(teamname)', function(assert){
    ...
    assert.equal(isStudentTeamNameValid(generateRandomString(TEAMNAME_MAX_LENGTH + 1)), false, "Exceed maximum number of characters - invalid");
});
...
QUnit.test('isStudentInputValid(editName, editTeamName, editEmail)', function(assert){
    ...
    assert.equal(isStudentInputValid("Billy", generateRandomString(TEAMNAME_MAX_LENGTH + 1), "d.Team@hotmail.com"), false, "Invalid teamname");
