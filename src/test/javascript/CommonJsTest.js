...
QUnit.test('setStatusMessage(message,error)', function(assert){	
    ...
    assert.ok(($("#statusMessage").attr("class") == "overflow-auto alert alert-warning"), "No warning");
...
QUnit.test('clearStatusMessage()', function(assert){
    ...
    assert.ok(($("#statusMessage").css('background-color') == "rgba(0, 0, 0, 0)" || $("#statusMessage").css('background-color') == "transparent"), "No background");
});
