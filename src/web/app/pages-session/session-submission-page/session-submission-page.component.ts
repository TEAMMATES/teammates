...
} from "../../../services/feedback-responses.service";
...
export class SessionSubmissionPageComponent implements OnInit, AfterViewInit {
  ...
  feedbackSessionSubmissionStatus: FeedbackSessionSubmissionStatus =
  ...
  recipientQuestionMap: Map<string, Set<number>> = new Map<
    string,
    Set<number>
  >();
  ...
  handleAutoSave(event: {
    ...
    this.autoSaveTimeout = setTimeout(() => {
      ...
      const clonedModel = {
        ...event.model,
        hasResponseChangedForRecipients: Array.from(
          event.model.hasResponseChangedForRecipients.entries()
        ),
        isTabExpandedForRecipients: Array.from(
          event.model.isTabExpandedForRecipients.entries()
        ),
      };
  ...
  loadAutoSavedData(questionId: string): void {
    ...
    if (savedModel) {
      const index = this.questionSubmissionForms.findIndex(
        (q) => q.feedbackQuestionId === questionId
      );
      if (index !== -1) {
        savedModel.hasResponseChangedForRecipients = new Map(
          savedModel.hasResponseChangedForRecipients
        );
        savedModel.isTabExpandedForRecipients = new Map(
          savedModel.isTabExpandedForRecipients
        );
    ...
    this.route.data
      ...
      .subscribe((queryParams: any) => {
        ...
        this.moderatedPerson = queryParams.moderatedperson
          ? queryParams.moderatedperson
          : "";
        this.previewAsPerson = queryParams.previewas
          ? queryParams.previewas
          : "";
        ...
        this.moderatedQuestionId = queryParams.moderatedquestionId
          ? queryParams.moderatedquestionId
          : "";
        ...
        this.authService.getAuthUser(undefined, nextUrl).subscribe({
          next: (auth: AuthInfo) => {
            const isPreviewOrModeration: boolean = !!(
              auth.user &&
            ...
            if (this.regKey && !isPreviewOrModeration) {
              this.authService
                .getAuthRegkeyValidity(this.regKey, this.intent)
                .subscribe({
    ...
    switch (this.intent) {
      case Intent.STUDENT_SUBMISSION:
        if (this.moderatedPerson || this.previewAsPerson) {
          ...
          request = this.courseService.getCourseAsStudent(
            this.courseId,
            this.regKey
          );
        }
      ...
      case Intent.INSTRUCTOR_SUBMISSION:
        request = this.courseService.getCourseAsInstructor(
          this.courseId,
          this.regKey
        );
        break;
  ...
  joinCourseForUnregisteredEntity(): void {
    this.navigationService.navigateByURL("/web/join", {
      ...
      .subscribe({
        next: (feedbackSession: FeedbackSession) => {
          ...
          this.formattedSessionClosingTime =
            this.getformattedSessionClosingTime(feedbackSession, TIME_FORMAT);
          ...
          if (!this.moderatedPerson) {
            ...
            switch (feedbackSession.submissionStatus) {
              ...
              case FeedbackSessionSubmissionStatus.OPEN:
                if (
                  this.isFeedbackEndingLessThanFifteenMinutes(feedbackSession)
                ) {
                  modalContent =
        ...
        next: (response: FeedbackQuestionsResponse) => {
          response.questions.forEach((feedbackQuestion: FeedbackQuestion) => {
            const model: QuestionSubmissionFormModel = {
              ...
              numberOfEntitiesToGiveFeedbackToSetting:
                feedbackQuestion.numberOfEntitiesToGiveFeedbackToSetting,
  ...
  trackQuestionSubmissionFormByFn(
    _: any,
    item: QuestionSubmissionFormModel
  ): any {
   ...
   */
  loadFeedbackQuestionRecipientsForQuestion(
    model: QuestionSubmissionFormModel
  ): void {
    this.feedbackQuestionsService
      ...
      .subscribe({
        next: (response: FeedbackQuestionRecipients) => {
          response.recipients.forEach(
            (recipient: FeedbackQuestionRecipient) => {
          ...
          if (!this.hasLoadedAllRecipients) {
            ...
            if (
              this.getQuestionSubmissionFormModeInDefaultView(model) ===
              ...
              model.recipientList.forEach(
                (recipient: FeedbackResponseRecipient) => {
                  this.addQuestionForRecipient(
          ...
          if (this.previewAsPerson) {
            ...
            const formMode: QuestionSubmissionFormMode =
              this.getQuestionSubmissionFormModeInDefaultView(model);
            model.recipientList.forEach(
              (recipient: FeedbackResponseRecipient) => {
                if (
                ...
                model.recipientSubmissionForms.push({
                  recipientIdentifier,
                  responseDetails:
                    this.feedbackResponsesService.getDefaultFeedbackResponseDetails(
                      model.questionType
                    ),
                  responseId: "",
                  isValid: true,
  ...
  getQuestionSubmissionFormMode(
    ...
    const isNumberOfEntitiesToGiveFeedbackToSettingLimited: boolean =
      ...
      model.numberOfEntitiesToGiveFeedbackToSetting ===
   ...
   */
  getQuestionSubmissionFormModeInDefaultView(
    model: QuestionSubmissionFormModel
  ): QuestionSubmissionFormMode {
    return this.getQuestionSubmissionFormMode(
      model,
      model.recipientList.length
    );
  }
      ...
      .pipe(
        finalize(() => {
          ...
          this.originalQuestionSubmissionForms.push({
            ...model,
            hasResponseChangedForRecipients: new Map(
              model.hasResponseChangedForRecipients
            ),
            isTabExpandedForRecipients: new Map(
              model.isTabExpandedForRecipients
            ),
            recipientList: model.recipientList.map((recipient) => ({
              ...recipient,
            })),
            recipientSubmissionForms: model.recipientSubmissionForms.map(
              (form) => ({
                ...form,
                responseDetails: { ...form.responseDetails },
                commentByGiver: form.commentByGiver
                  ? { ...form.commentByGiver }
                  : undefined,
              })
            ),
        ...
        next: (existingResponses: FeedbackResponsesResponse) => {
                ...
                const submissionForm: FeedbackResponseRecipientSubmissionFormModel =
                  {
                    ...
                    responseId: matchedExistingResponse
                      ? matchedExistingResponse.feedbackResponseId
                      : "",
                    isValid: true,
                  };
                if (
                  matchedExistingResponse &&
          ...
          if (
            ...
            let numberOfRecipientSubmissionFormsNeeded: number =
              model.customNumberOfEntitiesToGiveFeedbackTo -
              existingResponses.responses.length;

            existingResponses.responses.forEach(
              (response: FeedbackResponse) => {
                const submissionForm: FeedbackResponseRecipientSubmissionFormModel =
                  {
  ...
  get questionsNeedingSubmission(): QuestionSubmissionFormModel[] {
    return this.questionSubmissionForms.filter(
      (model: QuestionSubmissionFormModel) =>
        model.recipientSubmissionForms.length !== 0
    );
  }
  ...
  saveFeedbackResponses(
    ...
    questionSubmissionForms.forEach(
            ...
            this.feedbackResponsesService
              ...
              .pipe(
                tap((resp: FeedbackResponses) => {
                  ...
                  requestIds[questionSubmissionFormModel.feedbackQuestionId] =
                    resp.requestId || "";
                    ...
                    (
                      ...
                      if (
                        ...
                        recipientSubmissionFormModel.responseId =
                          correspondingResp.feedbackResponseId;
                        recipientSubmissionFormModel.responseDetails =
                          correspondingResp.responseDetails;
                        recipientSubmissionFormModel.recipientIdentifier =
                  ...
                  this.originalQuestionSubmissionForms.forEach(
                    (originalModel: QuestionSubmissionFormModel) => {
                      if (
                        originalModel.feedbackQuestionId ===
                        questionSubmissionFormModel.feedbackQuestionId
                      ) {
                        originalModel.recipientSubmissionForms.forEach(
                          (
                            originalRecipientSubmissionFormModel: FeedbackResponseRecipientSubmissionFormModel
                          ) => {
                            if (
                              responsesMap[
                                originalRecipientSubmissionFormModel
                                  .recipientIdentifier
                              ]
                            ) {
                              const correspondingResp: FeedbackResponse =
                              ...
                              originalRecipientSubmissionFormModel.responseId =
                                correspondingResp.feedbackResponseId;
                              originalRecipientSubmissionFormModel.responseDetails =
                                correspondingResp.responseDetails;
                              ...
                              originalRecipientSubmissionFormModel.responseId =
                                "";
                              originalRecipientSubmissionFormModel.commentByGiver =
                                undefined;
                            }
                          }
                        );
                ...
                catchError((error: ErrorMessageOutput) => {
                  failToSaveQuestions[
                    questionSubmissionFormModel.questionNumber
                  ] = error.error.message;
                  return of(error);
                })
              )
    ...
    forkJoin(savingRequests)
      .pipe(
        finalize(() => {
          ...
          modalRef.componentInstance.feedbackSessionName =
            this.feedbackSessionName;
          modalRef.componentInstance.feedbackSessionTimezone =
          ...
          modalRef.componentInstance.notYetAnsweredQuestions = Array.from(
            notYetAnsweredQuestions.values()
          );
          modalRef.componentInstance.failToSaveQuestions = failToSaveQuestions;

          if (recipientId) {
            this.questionSubmissionForms.forEach(
              (model: QuestionSubmissionFormModel) => {
                if (
                  this.recipientQuestionMap
                    .get(recipientId)!
                    .has(model.questionNumber)
                ) {
  ...
  createCommentRequest(
    ...
    if (
      !recipientSubmissionFormModel.commentByGiver.originalComment ||
      !isSameRecipient
    ) {
      // comment is new or original comment deleted because recipient has changed

      if (
        recipientSubmissionFormModel.commentByGiver.commentEditFormModel
          .commentText === ""
      ) {
  ...
  deleteParticipantComment(questionIndex: number, responseIdx: number): void {
    const recipientSubmissionFormModel: FeedbackResponseRecipientSubmissionFormModel =
      this.questionSubmissionForms[questionIndex].recipientSubmissionForms[
        responseIdx
      ];

    if (
      !recipientSubmissionFormModel.commentByGiver ||
      !recipientSubmissionFormModel.commentByGiver.originalComment
    ) {
      return;
    }
  ...
  loadRecipientsAndResponses(
    event: any,
    questionSubmissionForm: QuestionSubmissionFormModel
  ): void {
    if (
      event &&
      event.visible &&
      !questionSubmissionForm.isLoaded &&
      !questionSubmissionForm.isLoading
    ) {
  ...
  private getformattedSessionClosingTime(
    ...
    const userSessionEndingTime =
      DeadlineExtensionHelper.getUserFeedbackSessionEndingTimestamp(
        feedbackSession
      );
  ...
  private isFeedbackEndingLessThanFifteenMinutes(
    feedbackSession: FeedbackSession
  ): boolean {
    const userSessionEndingTime =
      DeadlineExtensionHelper.getOngoingUserFeedbackSessionEndingTimestamp(
        feedbackSession
      );
  ...
  saveResponsesForSelectedRecipientQuestions(
    ...
    const questionsToRecipient: Set<number> | undefined =
      this.recipientQuestionMap.get(recipientId);
    if (!questionsToRecipient) {
      this.statusMessageService.showErrorToast(
        "Failed to save response for this recipient. " +
  ...
  resetResponsesForSelectedRecipientQuestions(
    recipientId: string,
    questionSubmissionForms: QuestionSubmissionFormModel[]
  ): void {
    const questionsToRecipient: Set<number> | undefined =
    ...
    if (!questionsToRecipient) {
      this.statusMessageService.showErrorToast(
        "Failed to reset response for this recipient. " +
  ...
  resetFeedbackResponses(
    ...
    questionSubmissionForms.forEach(
      (questionSubmissionFormModel: QuestionSubmissionFormModel) => {
        const originalSubmissionForm =
          this.originalQuestionSubmissionForms.find(
            (originalModel: QuestionSubmissionFormModel) =>
              originalModel.feedbackQuestionId ===
              questionSubmissionFormModel.feedbackQuestionId
          );
        ...
        if (originalSubmissionForm) {
          if (recipientId) {
            questionSubmissionFormModel.recipientSubmissionForms.forEach(
              (form, index) => {
                if (form.recipientIdentifier === recipientId) {
                  const originalForm =
                    originalSubmissionForm.recipientSubmissionForms.find(
                      (originalRecipientForm) =>
                        originalRecipientForm.recipientIdentifier ===
                  ...
                  if (originalForm) {
                    questionSubmissionFormModel.recipientSubmissionForms[
                      ...
                      commentByGiver: originalForm.commentByGiver
                        ? { ...originalForm.commentByGiver }
                        : undefined,
                    };
                  }
                }
            ...
            questionSubmissionFormModel.hasResponseChangedForRecipients.set(
              recipientId,
              originalSubmissionForm.hasResponseChangedForRecipients.get(
                recipientId
              ) ?? false
            );
            questionSubmissionFormModel.isTabExpandedForRecipients.set(
              recipientId,
              originalSubmissionForm.isTabExpandedForRecipients.get(
                recipientId
              ) ?? true
            );

            if (savedData[questionSubmissionFormModel.feedbackQuestionId]) {
              const recipientIndex = savedData[
                questionSubmissionFormModel.feedbackQuestionId
              ].recipientSubmissionForms.findIndex(
              ...
              if (
                savedData[questionSubmissionFormModel.feedbackQuestionId]
                  .recipientSubmissionForms.length === 0
              ) {
                delete savedData[
                  questionSubmissionFormModel.feedbackQuestionId
                ];
            ...
            Object.assign(questionSubmissionFormModel, {
              ...
              recipientSubmissionForms:
                originalSubmissionForm.recipientSubmissionForms.map(
                  (form: FeedbackResponseRecipientSubmissionFormModel) => ({
                    ...form,
                    responseDetails: { ...form.responseDetails },
                    commentByGiver: form.commentByGiver
              ...
              hasResponseChangedForRecipients: new Map(
                originalSubmissionForm.hasResponseChangedForRecipients
              ),
              isTabExpandedForRecipients: new Map(
                originalSubmissionForm.isTabExpandedForRecipients
              ),
  ...
  hasResponseChangedForRecipient(
    recipientId: string,
    questionSubmissionForms: QuestionSubmissionFormModel[]
  ): boolean {
    const questionsToRecipient: Set<number> | undefined =
    ...
    return questionSubmissionForms.some(
      (questionSubmissionFormModel: QuestionSubmissionFormModel) =>
        questionsToRecipient.has(questionSubmissionFormModel.questionNumber) &&
        questionSubmissionFormModel.hasResponseChangedForRecipients.get(
          recipientId
        )
    );
  }
  ...
  groupQuestionsByRecipient(): void {
    ...
    forkJoin(recipientsObservables)
      .pipe(
        finalize(() => {
          this.ungroupableQuestionsSorted = Array.from(
            this.ungroupableQuestions
          ).sort();
      ...
      .subscribe({
        next: (feedbackQuestionRecipients: FeedbackQuestionRecipients[]) => {
          for (let i = 0; i < feedbackQuestionRecipients.length; i += 1) {
            const question: QuestionSubmissionFormModel =
              questionsToBeLoaded[i];
            ...
            if (
              this.getQuestionSubmissionFormMode(
                question,
                feedbackQuestionRecipients[i].recipients.length
              ) === QuestionSubmissionFormMode.FIXED_RECIPIENT &&
              ...
              for (
                let j = 0;
                j < feedbackQuestionRecipients[i].recipients.length;
                j += 1
              ) {
                const recipient: FeedbackQuestionRecipient =
                  feedbackQuestionRecipients[i].recipients[j];
                this.addQuestionForRecipient(
        ...
        error: () => {
          this.statusMessageService.showWarningToast(
            "Failed to build groupable questions"
          );
        },
      });
  }
  ...
  getRecipientName(recipientIdentifier: string): string {
    const question: QuestionSubmissionFormModel | undefined =
      this.questionSubmissionForms.find(
        (model: QuestionSubmissionFormModel) =>
          model.questionNumber ===
          this.recipientQuestionMap.get(recipientIdentifier)!.values().next()
            .value
      );
    ...
    const recipient: FeedbackResponseRecipient | undefined =
      question!.recipientList.find(
        (r: FeedbackResponseRecipient) =>
          r.recipientIdentifier === recipientIdentifier
      );
