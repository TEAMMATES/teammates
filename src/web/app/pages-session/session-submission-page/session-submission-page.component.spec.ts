...
describe("SessionSubmissionPageComponent", () => {
  const deepCopy: <T>(obj: T) => T = <T>(obj: T) =>
  ...
  const testMcqQuestionSubmissionForm: QuestionSubmissionFormModel = {
    ...
    recipientList: [
      { recipientName: "Gene Harris", recipientIdentifier: "gene-harris-id" },
    ],
  ...
  const testMsqQuestionSubmissionForm: QuestionSubmissionFormModel = {
    ...
    recipientList: [
      { recipientName: "Barry Harris", recipientIdentifier: "barry-harris-id" },
    ],
  ...
  beforeEach(() => {
    ...
    feedbackResponseCommentService = TestBed.inject(
      FeedbackResponseCommentService
    );
  ...
  it("should snap with feedback session and user details", () => {
    ...
    component.feedbackSessionInstructions =
  ...
  it("should verify allowed access with used reg key", () => {
    ...
    jest.spyOn(authService, "getAuthUser").mockReturnValue(of(testInfo));
    jest
      .spyOn(authService, "getAuthRegkeyValidity")
      .mockReturnValue(of(testValidity));
    const navSpy: SpyInstance = jest
      .spyOn(navService, "navigateByURLWithParamEncoding")
      .mockImplementation();
    ...
    expect(navSpy).toHaveBeenLastCalledWith(
      "/web/student/sessions/submission",
  ...
  it("should deny unallowed access with valid reg key for logged in user", () => {
    ...
    jest
      ...
      .mockReturnValue(of(testValidity));
    const navSpy: SpyInstance = jest
      .spyOn(navService, "navigateWithErrorMessage")
      .mockImplementation();
  ...
  it("should load an instructor name", () => {
    ...
    jest
      .spyOn(instructorService, "getInstructor")
      .mockReturnValue(of(testInstructor));
    component.loadPersonName();
  ...
  it("should join course for unregistered student", () => {
    const navSpy: SpyInstance = jest
      .spyOn(navService, "navigateByURL")
      .mockImplementation();
  ...
  it("should load an open feedback session", () => {
    ...
    expect(component.feedbackSessionInstructions).toEqual(
      testOpenFeedbackSession.instructions
    );
    expect(component.feedbackSessionSubmissionStatus).toEqual(
      testOpenFeedbackSession.submissionStatus
    );
    expect(component.feedbackSessionTimezone).toEqual(
      testOpenFeedbackSession.timeZone
    );
    expect(component.isSubmissionFormsDisabled).toEqual(false);
  });

  it("should load a closed feedback session", () => {
    const testClosedFeedbackSession: FeedbackSession = deepCopy(
      testOpenFeedbackSession
    );
    testClosedFeedbackSession.submissionStatus =
      FeedbackSessionSubmissionStatus.CLOSED;
    const fsSpy: SpyInstance = jest
      .spyOn(feedbackSessionsService, "getFeedbackSession")
    ...
    expect(component.feedbackSessionInstructions).toEqual(
      testClosedFeedbackSession.instructions
    );
    expect(component.feedbackSessionSubmissionStatus).toEqual(
      testClosedFeedbackSession.submissionStatus
    );
    expect(component.feedbackSessionTimezone).toEqual(
      testClosedFeedbackSession.timeZone
    );
    expect(component.isSubmissionFormsDisabled).toEqual(true);
  });

  it("should load a visible not open feedback session", () => {
    const testVisibleNotOpenFeedbackSession: FeedbackSession = deepCopy(
      testOpenFeedbackSession
    );
    testVisibleNotOpenFeedbackSession.submissionStatus =
    ...
    expect(component.feedbackSessionInstructions).toEqual(
      testVisibleNotOpenFeedbackSession.instructions
    );
    expect(component.feedbackSessionSubmissionStatus).toEqual(
      testVisibleNotOpenFeedbackSession.submissionStatus
    );
    expect(component.feedbackSessionTimezone).toEqual(
      testVisibleNotOpenFeedbackSession.timeZone
    );
    expect(component.isSubmissionFormsDisabled).toEqual(true);
  });
  ...
  it("should load feedback questions", () => {
      ...
      questions: [
        {
          ...
          questionDescription:
            testMcqQuestionSubmissionForm2.questionDescription,
          ...
          showRecipientNameTo:
            testMcqQuestionSubmissionForm2.showRecipientNameTo,
        },
      ],
    };
    ...
    expect(component.questionSubmissionForms[0]).toEqual(
      testMcqQuestionSubmissionForm2
    );
  ...
  it("should load the recipients and responses of a question if not yet loaded", () => {
    ...
    component.loadRecipientsAndResponses(
      { visible: true },
      testMcqQuestionSubmissionForm2
    );
    ...
    expect(testMcqQuestionSubmissionForm2.recipientSubmissionForms).toEqual([
      testMcqRecipientSubmissionForm3,
      testMcqRecipientSubmissionForm4,
    ]);
  ...
  it("should not load the recipients and responses of a question if already loaded", () => {
    const loadRecipientsSpy: SpyInstance = jest.spyOn(
      feedbackQuestionsService,
      "loadFeedbackQuestionRecipients"
    );
    const getResponseSpy: SpyInstance = jest.spyOn(
  ...
  it("should check that there are no responses to submit", () => {
    const testSubmissionForm: QuestionSubmissionFormModel = deepCopy(
      testTextQuestionSubmissionForm
    );
  ...
  it("should save feedback responses", () => {
    ...
    const testResponseDetails1: any = deepCopy(
      testMcqRecipientSubmissionForm.responseDetails
    );
    // leave question unanswered
    const testResponseDetails2: FeedbackTextResponseDetails = {
      answer: "",
      questionType: FeedbackQuestionType.TEXT,
    };
    const testQuestionSubmissionForm1: QuestionSubmissionFormModel = deepCopy(
      testMcqQuestionSubmissionForm
    );
    const testQuestionSubmissionForm2: QuestionSubmissionFormModel = deepCopy(
      testTextQuestionSubmissionForm
    );
    testQuestionSubmissionForm1.recipientSubmissionForms[0].responseDetails =
      testResponseDetails1;
    testQuestionSubmissionForm2.recipientSubmissionForms[0].responseDetails =
      testResponseDetails2;
    component.questionSubmissionForms = [
    ...
    expect(mockModalRef.componentInstance.notYetAnsweredQuestions).toHaveLength(
      1
    );
  ...
  it("should not save invalid feedback responses", () => {
    ...
    const testResponseDetails2: any = deepCopy(
      testConstsumRecipientSubmissionForm.responseDetails
    );
    const testQuestionSubmissionForm1: QuestionSubmissionFormModel = deepCopy(
      testMcqQuestionSubmissionForm
    );
    ...
    expect(mockModalRef.componentInstance.failToSaveQuestions).toEqual({
      [testQuestionSubmissionForm2.questionNumber]:
  ...
  it("should create comment request to create new comment when submission form has no original comment", () => {
    const testSubmissionForm: FeedbackResponseRecipientSubmissionFormModel =
      deepCopy(testTextRecipientSubmissionForm);
    ...
    component.createCommentRequest(testSubmissionForm).subscribe(() => {
      expect(testSubmissionForm.commentByGiver).toEqual(
        component.getCommentModel(
          testComment,
          testSubmissionForm.recipientIdentifier
        )
      );
    });
  ...
  it(
    ...
    () => {
      const testSubmissionForm: FeedbackResponseRecipientSubmissionFormModel =
        deepCopy(testMcqRecipientSubmissionForm);
      testSubmissionForm.commentByGiver!.originalRecipientIdentifier =
        "other-recipient-identifier";
      const commentSpy: SpyInstance = jest
        .spyOn(feedbackResponseCommentService, "createComment")
        .mockReturnValue(of(testComment));
  ...
  it("should create comment request to update existing comment when submission form has original comment", () => {
    ...
    const expectedId: any =
      testMcqRecipientSubmissionForm.commentByGiver?.originalComment
  ...
  it("should create comment request to delete existing comment when new comment text is empty", () => {
    const testSubmissionForm: FeedbackResponseRecipientSubmissionFormModel =
      deepCopy(testMcqRecipientSubmissionForm2);
    const expectedId: any =
      testMcqRecipientSubmissionForm2.commentByGiver?.originalComment
        ?.feedbackResponseCommentId;
    const commentSpy: SpyInstance = jest
  ...
  it("should delete participant comment", () => {
    ...
    const expectedId: any =
      testMsqQuestionSubmissionForm.recipientSubmissionForms[0].commentByGiver
  ...
  it("should autosave data to localStorage", () => {
    ...
    const model: QuestionSubmissionFormModel = deepCopy(
      testMcqQuestionSubmissionForm
    );
    model.hasResponseChangedForRecipients = new Map<string, boolean>().set(
      "r1",
      true
    );
    model.isTabExpandedForRecipients = new Map<string, boolean>().set(
      "r1",
      true
    );
  ...
  it("should load autosaved data from localStorage", () => {
    ...
    savedModel.hasResponseChangedForRecipients = Array.from(
      new Map<string, boolean>().set("r1", true).entries()
    );
    savedModel.isTabExpandedForRecipients = Array.from(
      new Map<string, boolean>().set("r1", true).entries()
    );
    ...
    expect(
      component.questionSubmissionForms[0].hasResponseChangedForRecipients.get(
        "r1"
      )
    ).toBe(true);
    expect(
      component.questionSubmissionForms[0].isTabExpandedForRecipients.get("r1")
    ).toBe(true);
