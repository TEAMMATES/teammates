...
export class AdminSearchPageComponent {
  ...
  /**
   * Resets the instructor's linked account for the course.
   */
  resetInstructorAccount(instructor: InstructorAccountSearchResult, event: any): void {
    if (event) {
      event.preventDefault();
      event.stopPropagation();
    }

    const modalContent: string = `Are you sure you want to reset the account link currently associated for
        <strong>${instructor.name}</strong> in the course <strong>${instructor.courseId}</strong>?
        The user will need to sign in again and rejoin the course.`;
    const modalRef: NgbModalRef = this.simpleModalService.openConfirmationModal(
        `Reset <strong>${instructor.name}</strong>'s account link?`, SimpleModalType.WARNING, modalContent);

    modalRef.result.then(() => {
      this.accountService.resetInstructorAccount(instructor.courseId, instructor.email).subscribe({
        next: () => {
          this.search();
          this.statusMessageService.showSuccessToast('The instructor\'s account has been reset.');
        },
        error: (resp: ErrorMessageOutput) => {
          this.statusMessageService.showErrorToast(resp.error.message);
        },
  ...
  /**
   * Resets the student's linked account for the course.
   */
  resetStudentAccount(student: StudentAccountSearchResult, event: any): void {
    if (event) {
      event.preventDefault();
      event.stopPropagation();
    }
    const modalContent: string = `Are you sure you want to reset the account link currently associated for
        ...
        The user will need to sign in again and rejoin the course.`;
    const modalRef: NgbModalRef = this.simpleModalService.openConfirmationModal(
        `Reset <strong>${student.name}</strong>'s account link?`, SimpleModalType.WARNING, modalContent);

    modalRef.result.then(() => {
      this.accountService.resetStudentAccount(student.courseId, student.email).subscribe({
        next: () => {
          student.accountId = '';
          this.statusMessageService.showSuccessToast('The student\'s account has been reset.');
        },
        error: (resp: ErrorMessageOutput) => {
          this.statusMessageService.showErrorToast(resp.error.message);
        },
