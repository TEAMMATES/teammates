...
export class AdminEmailTemplatesPageComponent implements OnInit {
  ...
  private originalSubject: string = '';
  private originalBody: string = '';

  get isUnchanged(): boolean {
    return this.model.subject === this.originalSubject
        && this.model.body === this.originalBody;
  }
    ...
    this.emailTemplateService.getEmailTemplate(templateKey)
      ...
      .subscribe({
        next: (template: EmailTemplate) => {
          ...
          this.originalSubject = template.subject;
          this.originalBody = template.body;
        },
        error: (resp: ErrorMessageOutput) => {
          this.statusMessageService.showErrorToast(resp.error.message);
        },
  ...
  saveTemplate(): void {
    if (this.isUnchanged) {
      this.statusMessageService.showWarningToast('No changes made to save.');
      return;
    }
    this.isSaving = true;
    ...
    this.emailTemplateService.updateEmailTemplate(request)
      ...
      .subscribe({
        next: (template: EmailTemplate) => {
          ...
          this.originalSubject = this.model.subject;
          this.originalBody = this.model.body;
  ...
  revertToDefault(): void {
    ...
    this.emailTemplateService.updateEmailTemplate(request)
      .pipe(finalize(() => { this.isReverting = false; }))
      .subscribe({
        next: (template: EmailTemplate) => {
          this.model = {
            subject: template.subject,
