import { FeedbackMsqResponseDetails, FeedbackQuestionType } from '../../../../../types/api-output';
import { Response } from '../../../../../types/question-statistics.model';

export const msqQuestionResponses = {
  responsesNoOther: [
    {
      giver: 'Alice',
      giverTeam: 'Team 1',
      giverEmail: 'alice@gmail.com',
      giverSection: '',
      recipient: 'Alice',
      recipientTeam: 'Team 1',
      recipientEmail: 'alice@gmail.com',
      recipientSection: '',
      responseDetails: {
        answers: ['optionA', 'optionB'],
        isOther: false,
        otherFieldContent: 'nothing',
        questionType: FeedbackQuestionType.MSQ,
      },
    },
    {
      giver: 'Bob',
      giverTeam: 'Team 2',
      giverEmail: 'bob@gmail.com',
      giverSection: '',
      recipient: 'Bob',
      recipientTeam: 'Team 2',
      recipientEmail: 'bob@gmail.com',
      recipientSection: '',
      responseDetails: {
        answers: ['optionA'],
        isOther: false,
        otherFieldContent: 'nothing',
        questionType: FeedbackQuestionType.MSQ,
      },
    },
    {
      giver: 'Charles',
      giverTeam: 'Team 1',
      giverEmail: 'charles@gmail.com',
      giverSection: '',
      recipient: 'Charles',
      recipientTeam: 'Team 1',
      recipientEmail: 'charles@gmail.com',
      recipientSection: '',
      responseDetails: {
        answers: [''],
        isOther: false,
        otherFieldContent: 'nothing',
        questionType: FeedbackQuestionType.MSQ,
      },
    },
  ] as Response<FeedbackMsqResponseDetails>[],
  responsesWithOther: [
    {
      giver: 'Alice',
      giverTeam: 'Team 1',
      giverEmail: 'alice@gmail.com',
      giverSection: '',
      recipient: 'Alice',
      recipientTeam: 'Team 1',
      recipientEmail: 'alice@gmail.com',
      recipientSection: '',
      responseDetails: {
        answers: ['optionA', 'optionB', 'optionC'],
        isOther: false,
        otherFieldContent: 'nothing',
        questionType: FeedbackQuestionType.MSQ,
      },
    },
    {
      giver: 'Bob',
      giverTeam: 'Team 2',
      giverEmail: 'bob@gmail.com',
      giverSection: '',
      recipient: 'Bob',
      recipientTeam: 'Team 2',
      recipientEmail: 'bob@gmail.com',
      recipientSection: '',
      responseDetails: {
        answers: ['optionA', 'etcetra'],
        isOther: true,
        otherFieldContent: 'nothing',
        questionType: FeedbackQuestionType.MSQ,
      },
    },
    {
      giver: 'Charles',
      giverTeam: 'Team 1',
      giverEmail: 'charles@gmail.com',
      giverSection: '',
      recipient: 'Charles',
      recipientTeam: 'Team 1',
      recipientEmail: 'charles@gmail.com',
      recipientSection: '',
      responseDetails: {
        answers: [''],
        isOther: false,
        otherFieldContent: 'nothing',
        questionType: FeedbackQuestionType.MSQ,
      },
    },
  ] as Response<FeedbackMsqResponseDetails>[],
  expectedPerRecipientResponses: {
    'alice@gmail.com': {
      average: 1.5,
      recipient: 'Alice',
      recipientEmail: 'alice@gmail.com',
      recipientTeam: 'Team 1',
      responses: {
        optionA: 1,
        optionB: 1,
        optionC: 0,
      },
      total: 3,
    },
    'bob@gmail.com': {
      average: 1,
      recipient: 'Bob',
      recipientEmail: 'bob@gmail.com',
      recipientTeam: 'Team 2',
      responses: {
        optionA: 1,
        optionB: 0,
        optionC: 0,
      },
      total: 1,
    },
    'charles@gmail.com': {
      average: 0,
      recipient: 'Charles',
      recipientEmail: 'charles@gmail.com',
      recipientTeam: 'Team 1',
      responses: {
        optionA: 0,
        optionB: 0,
        optionC: 0,
      },
      total: 0,
    },
  },
  expectedPerRecipientResponsesWithOther: {
    'alice@gmail.com': {
      average: 2,
      recipient: 'Alice',
      recipientEmail: 'alice@gmail.com',
      recipientTeam: 'Team 1',
      responses: {
        Other: 0,
        optionA: 1,
        optionB: 1,
        optionC: 1,
      },
      total: 6,
    },
    'bob@gmail.com': {
      average: 2.5,
      recipient: 'Bob',
      recipientEmail: 'bob@gmail.com',
      recipientTeam: 'Team 2',
      responses: {
        Other: 1,
        optionA: 1,
        optionB: 0,
        optionC: 0,
      },
      total: 5,
    },
    'charles@gmail.com': {
      average: 0,
      recipient: 'Charles',
      recipientEmail: 'charles@gmail.com',
      recipientTeam: 'Team 1',
      responses: {
        Other: 0,
        optionA: 0,
        optionB: 0,
        optionC: 0,
      },
      total: 0,
    },
  },
} satisfies {
  responsesNoOther: Response<FeedbackMsqResponseDetails>[];
  responsesWithOther: Response<FeedbackMsqResponseDetails>[];
  expectedPerRecipientResponses: Record<string, unknown>;
  expectedPerRecipientResponsesWithOther: Record<string, unknown>;
};

export default msqQuestionResponses;
