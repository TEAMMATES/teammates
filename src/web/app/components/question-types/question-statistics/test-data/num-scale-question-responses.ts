import { FeedbackNumericalScaleResponseDetails, FeedbackQuestionType } from '../../../../../types/api-output';
import { Response } from '../../../../../types/question-statistics.model';

export const numScaleQuestionResponses = {
  responses: [
    {
      giver: 'Alice',
      giverTeam: 'Team 1',
      giverEmail: 'alice@gmail.com',
      giverSection: '',
      recipient: 'Instructor',
      recipientTeam: 'Instructors',
      recipientEmail: '',
      recipientSection: '',
      responseDetails: {
        answer: 5,
        questionType: FeedbackQuestionType.NUMSCALE,
      },
    },
    {
      giver: 'Bob',
      giverTeam: 'Team 2',
      giverEmail: 'bob@gmail.com',
      giverSection: '',
      recipient: 'Instructor',
      recipientTeam: 'Instructors',
      recipientEmail: '',
      recipientSection: '',
      responseDetails: {
        answer: 2,
        questionType: FeedbackQuestionType.NUMSCALE,
      },
    },
    {
      giver: 'Charles',
      giverTeam: 'Team 1',
      giverEmail: 'charles@gmail.com',
      giverSection: '',
      recipient: 'Instructor',
      recipientTeam: 'Instructors',
      recipientEmail: '',
      recipientSection: '',
      responseDetails: {
        answer: 1,
        questionType: FeedbackQuestionType.NUMSCALE,
      },
    },
  ] as Response<FeedbackNumericalScaleResponseDetails>[],
  responsesAtZero: [
    {
      giver: 'Alice',
      giverTeam: 'Team 1',
      giverEmail: 'alice@gmail.com',
      giverSection: '',
      recipient: 'Instructor',
      recipientTeam: 'Instructors',
      recipientEmail: '',
      recipientSection: '',
      responseDetails: {
        answer: 0,
        questionType: FeedbackQuestionType.NUMSCALE,
      },
    },
    {
      giver: 'Bob',
      giverTeam: 'Team 2',
      giverEmail: 'bob@gmail.com',
      giverSection: '',
      recipient: 'Instructor',
      recipientTeam: 'Instructors',
      recipientEmail: '',
      recipientSection: '',
      responseDetails: {
        answer: 0,
        questionType: FeedbackQuestionType.NUMSCALE,
      },
    },
    {
      giver: 'Charles',
      giverTeam: 'Team 1',
      giverEmail: 'charles@gmail.com',
      giverSection: '',
      recipient: 'Instructor',
      recipientTeam: 'Instructors',
      recipientEmail: '',
      recipientSection: '',
      responseDetails: {
        answer: 0,
        questionType: FeedbackQuestionType.NUMSCALE,
      },
    },
  ] as Response<FeedbackNumericalScaleResponseDetails>[],
  responsesWithSelf: [
    {
      giver: 'Alice',
      giverTeam: 'Team 1',
      giverEmail: 'alice@gmail.com',
      giverSection: '',
      recipient: 'Instructor',
      recipientTeam: 'Instructors',
      recipientEmail: '',
      recipientSection: '',
      responseDetails: {
        answer: 2,
        questionType: FeedbackQuestionType.NUMSCALE,
      },
    },
    {
      giver: 'Bob',
      giverTeam: 'Team 2',
      giverEmail: 'bob@gmail.com',
      giverSection: '',
      recipient: 'Instructor',
      recipientTeam: 'Instructors',
      recipientEmail: '',
      recipientSection: '',
      responseDetails: {
        answer: 3,
        questionType: FeedbackQuestionType.NUMSCALE,
      },
    },
    {
      giver: 'Charles',
      giverTeam: 'Team 1',
      giverEmail: 'charles@gmail.com',
      giverSection: '',
      recipient: 'Instructor',
      recipientTeam: 'Instructors',
      recipientEmail: '',
      recipientSection: '',
      responseDetails: {
        answer: 4,
        questionType: 'FeedbackQuestionType.NUMSCALE',
      },
    },
    {
      giver: 'Instructor',
      giverTeam: 'Instructors',
      giverEmail: '',
      giverSection: '',
      recipient: 'Instructor',
      recipientTeam: 'Instructors',
      recipientEmail: '',
      recipientSection: '',
      responseDetails: {
        answer: 5,
        questionType: 'FeedbackQuestionType.NUMSCALE',
      },
    },
  ] as Response<FeedbackNumericalScaleResponseDetails>[],
} satisfies {
  responses: Response<FeedbackNumericalScaleResponseDetails>[];
  responsesAtZero: Response<FeedbackNumericalScaleResponseDetails>[];
  responsesWithSelf: Response<FeedbackNumericalScaleResponseDetails>[];
};

export default numScaleQuestionResponses;
