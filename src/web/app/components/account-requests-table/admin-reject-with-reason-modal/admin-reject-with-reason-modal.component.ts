...
export class RejectWithReasonModalComponent implements OnInit {
  ...
  + '<strong>Remedy:</strong> You can log in to TEAMMATES with your existing account </p>\n\n'
  + '<p>If you are logged into multiple browser profiles or identity sessions, try an incognito/private window '
  + 'or sign out of other accounts first. Let us know (with a screenshot) if that doesn\'t work.</p>'
  ...
  ngOnInit(): void {
    ...
    this.replaceAccountIdPlaceholder();
  }
  ...
  replaceAccountIdPlaceholder(): void {
    ...
    .subscribe({
      next: (resp: AdminSearchResult) => {
        ...
        if (!hasInstructors) {
          this.rejectionReasonBody = this.rejectionReasonBody.replace('{accountId}', 'NO_ACCOUNT_ID');
          return;
        }
        ...
        if (this.existingAccount.accountId === '') {
          // When an instructor account exists, but for some reason does not have a accountId
          this.rejectionReasonBody = this.rejectionReasonBody.replace('{accountId}', 'NO_ACCOUNT_ID');
        } else {
