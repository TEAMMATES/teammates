...
describe('SearchTermsHighlighterPipe', () => {
  ...
  it('should highlight text by case insensitive word match', () => {
    ...
    for (const consolidatedSample of consolidatedSamples) {
      expect(
          highlighterPipe.transform(consolidatedSample.sampleValue, consolidatedSample.sampleSearch),
      ).toEqual(consolidatedSample.expected);
    }
  });
  ...
  it('should not highlight text if there is no match', () => {
    ...
    for (const consolidatedSample of consolidatedSamples) {
      expect(
          highlighterPipe.transform(consolidatedSample.sampleValue, consolidatedSample.sampleSearch),
      ).toEqual(consolidatedSample.sampleValue);
    }
  });
  ...
  it('should highlight text if there are exact phrases', () => {
    ...
    expect(highlighterPipe.transform(sampleValue, sampleSearch)).toBe(
      '<span class="highlighted-text">student one</span> ' +
  ...
  it('should highlight case-insensitive partial matches if partial parameter is specified', () => {
    const consolidatedSamples = [
      ...
      },
      {
        ...
        sampleValue: 'TestCourse',
        expected:
          'T<span class="highlighted-text">e</span>stCou<span class="highlighted-text">' +
          'r</span>s<span class="highlighted-text">e</span>',
      },
    ];
    for (const consolidatedSample of consolidatedSamples) {
      expect(
          highlighterPipe.transform(consolidatedSample.sampleValue, consolidatedSample.sampleSearch, true),
      ).toEqual(consolidatedSample.expected);
    }
  });
  ...
  it(
    'should highlight case-insensitive partial matches with exact phrases in search term ' +
