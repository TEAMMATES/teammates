...
export abstract class InstructorSessionModalPageComponent extends InstructorSessionBasePageComponent {
    ...
    forkJoin([
      ...
      .subscribe({
        next: (result: any[]) => {
          ...
          const instructors: Instructor[] = (result[1] as Instructors)
            .instructors;
   ...
   */
  sendRemindersToRespondentsEventHandler(
    model: SessionsTableRowModel,
    selectAllRespondents: boolean
  ): void {
    this.isSendReminderLoading = true;
    const courseId: string = model.feedbackSession.courseId;
    const feedbackSessionName: string =
    ...
    forkJoin([
      ...
      .subscribe({
        next: (result: any[]) => {
          ...
          const instructors: Instructor[] = (result[2] as Instructors)
            .instructors;
