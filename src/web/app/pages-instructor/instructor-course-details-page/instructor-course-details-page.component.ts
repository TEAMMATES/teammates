...
export class InstructorCourseDetailsPageComponent implements OnInit {
  ...
  openRemindStudentModal(): void {
    ...
    this.simpleModalService
      .openConfirmationModal('Sending join requests?', SimpleModalType.INFO, modalContent)
      .result.then(
        () => {
  ...
  openDeleteAllStudentsConfirmationModal(): void {
    ...
    this.simpleModalService
      .openConfirmationModal('Delete all students?', SimpleModalType.DANGER, modalContent)
      .result.then(
        () => {
    ...
    this.courseService.removeStudentFromCourse(this.courseDetails.course.courseId, studentEmail).subscribe({
      next: () => {
        ...
        this.statusMessageService.showSuccessToast(
          `Student is successfully deleted from course "${this.courseDetails.course.courseId}"`,
        );
      },
      error: (resp: ErrorMessageOutput) => {
        this.statusMessageService.showErrorToast(resp.error.message);
