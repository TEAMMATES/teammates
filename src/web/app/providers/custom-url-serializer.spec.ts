...
describe('Custom Url Serializer', () => {
  ...
  it('should convert plus sign to white space using default url serializer', () => {
    ...
    const urlTree: UrlTree = dus.parse('/localhost:8080/web/instructor/sessions/edit?courseid=C01&fsname=test+session');
  ...
  it('should preserve the plus sign in the url using custom url serializer', () => {
    ...
    const urlTree: UrlTree = serializer.parse(
      '/localhost:8080/web/instructor/sessions/edit?courseid=C01&fsname=test+session',
    );
