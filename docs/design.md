...
Automated requests are all requests sent automatically by the cron scheduler / task queue during specific periods of time.
...
1. The source of the request will be checked for administrator privilege. If this privilege is absent (e.g. non-administrator users trying to invoke the automated actions), the request will be dropped and a `403 Forbidden` status will be returned.
   - Requests from external schedulers (cron) and Cloud Tasks (workers) are authenticated via `Authorization: Bearer <token>` and granted admin privilege.
   - Administrators can manually invoke these requests with the bearer token; this is particularly useful in testing the actions associated with those requests.
...
<puml src="diagrams/TestDriverComponent.puml"/>
