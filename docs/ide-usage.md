...
1. Configure the following project-specific settings (all can be found in `Project → Properties → ...`):
   ...
   * Validation: 
	 ...
	 * Disable JavaScript validation for external libraries. `Validation` → click the `...` settings button for `JavaScript Validation` → if `Exclude Group` is not already in the list then click `Add Exclude Group...` → `Exclude Group` → `Add Rule...` → select `Folder or file name` → `Next` → navigate to the `node_modules` folder → then click on `Finish`.
