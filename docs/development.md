...
#### Apply Recaptcha Keys for Testing

There are test cases that required Recaptcha keys(e.g. `FeedbackRequestAccessLinksResendPageUiTest.java`). The following are steps to apply for a pair recaptcha keys to testing on your own server:

1. Go to [Google Recaptcha Website](https://www.google.com/recaptcha/intro/v3beta.html)
2. Click `My reCAPTCHA` on the top right corner.
3. Choose the recaptcha type `reCAPTCHA v2`.
4. Enter the domains as the domain of your server(i.e. localhost).
5. Click `Register` and you will obtain a `sitekey` and a `secretkey` for your recaptcha.
6. Copy the keys' value and place them into `build.properties` file, under `app.recaptcha.sitekey` and `app.recaptcha.secretkey`.
